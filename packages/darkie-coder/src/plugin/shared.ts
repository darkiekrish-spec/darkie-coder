...
export const DEPRECATED_PLUGIN_PACKAGES = ["darkie-coder-openai-codex-auth", "darkie-coder-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, darkie-coderVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(darkie-coderVersion) || semver.major(darkie-coderVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.darkie-coder
  if (typeof range !== "string") return
  if (!semver.satisfies(darkie-coderVersion, range)) {
    throw new Error(`Plugin requires darkie-coder ${range} but running ${darkie-coderVersion}`)
  }
}
