...
export type PatchDeps = {
  ...
  files: (dir: string, name: "darkie-coder" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".darkie-coder")
}

function patchName(kind: Kind): "darkie-coder" | "tui" {
  if (kind === "server") return "darkie-coder"
  return "tui"
}
