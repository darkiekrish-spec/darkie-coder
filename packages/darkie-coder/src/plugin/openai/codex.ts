...
export async function CodexAuthPlugin(input: PluginInput, options: CodexAuthPluginOptions = {}): Promise<Hooks> {
  ...
  return {
    ...
    "chat.headers": async (input, output) => {
      ...
      output.headers.originator = "darkie-coder"
      output.headers["User-Agent"] = `darkie-coder/${InstallationVersion} (${os.platform()} ${os.release()}; ${os.arch()})`
