...
import type { AssistantMessage, Message, Darkie CoderClient, Session, SessionMessageResponse } from "@opencode-ai/sdk/v2"
...
export const AuthMethodID = "darkie-coder-login"
...
export class Service extends Context.Service<Service, Interface>()("@darkie-coder/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `darkie-coder auth login` in the terminal",
      name: "Login with darkie-coder",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: Darkie CoderClient) {
...
function makeUsageService(sdk: Darkie CoderClient) {
...
async function loadDirectorySnapshot(sdk: Darkie CoderClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, darkie-coder provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const darkie-coderProvider = providers[ProviderV2.ID.make("darkie-coder")]
  const darkie-coderModel = darkie-coderProvider ? Provider.sort(Object.values(darkie-coderProvider.models))[0] : undefined
  if (darkie-coderProvider && darkie-coderModel) return { providerID: darkie-coderProvider.id, modelID: darkie-coderModel.id }
...
function promptErrorMessage(error: AssistantError) {
  ...
  return "Darkie Coder prompt failed"
}
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Darkie Coder service failure", service })
}
