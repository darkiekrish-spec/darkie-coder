...
declare global {
  const DARKIE_CODER_WORKER_PATH: string
}
...
async function target() {
  if (typeof DARKIE_CODER_WORKER_PATH !== "undefined") return DARKIE_CODER_WORKER_PATH
...
export const TuiThreadCommand = cmd({
  command: "$0 [project]",
  describe: "start darkie-coder tui",
  builder: (yargs) =>
    withNetworkOptions(yargs)
      .positional("project", {
        type: "string",
        describe: "path to start darkie-coder in",
      })
      .option("model", {
        type: "string",
        alias: ["m"],
  ...
  handler: async (args) => {
    ...
    try {
      ...
      const transport = external
        ...
        : {
            url: "http://darkie-coder.internal",
