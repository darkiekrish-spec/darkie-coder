...
// CLI entry point for `darkie-coder run` and `darkie-coder --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`darkie-coder --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`darkie-coder --mini --attach`): connects to a running
//      darkie-coder server and runs interactive mode against it.
//
...
import { createDarkie CoderClient, type Darkie CoderClient, type ToolPart } from "@opencode-ai/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<Darkie CoderClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run darkie-coder with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running darkie-coder server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to DARKIE_CODER_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to DARKIE_CODER_SERVER_USERNAME or 'darkie-coder')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      const attachSDK = (dir?: string) => {
        return createDarkie CoderClient({
          baseUrl: args.attach!,
          directory: dir,
      ...
      async function session(sdk: Darkie CoderClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: Darkie CoderClient, sessionID: string) {
      ...
      async function current(sdk: Darkie CoderClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: Darkie CoderClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: Darkie CoderClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: Darkie CoderClient) {
        ...
        async function loop(client: Darkie CoderClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createDarkie CoderClient({
        baseUrl: "http://darkie-coder.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
