...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall darkie-coder and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall Darkie Coder")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g darkie-coder-ai",
      pnpm: "pnpm uninstall -g darkie-coder-ai",
      bun: "bun remove -g darkie-coder-ai",
      yarn: "yarn global remove darkie-coder-ai",
      brew: "brew uninstall darkie-coder",
      choco: "choco uninstall darkie-coder",
      scoop: "scoop uninstall darkie-coder",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "darkie-coder", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (binDir.includes(".darkie-coder")) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using Darkie Coder!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# darkie-coder") || content.includes(".darkie-coder/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".darkie-coder/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".darkie-coder/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".darkie-coder"))
    ) {
      continue
    }
