...
//
// Enable with DARKIE_CODER_DIRECT_TRACE=1. Writes one JSON line per event to
// ~/.local/share/darkie-coder/log/direct/<timestamp>-<pid>.jsonl. Also writes
...
export function trace(): Trace | undefined {
  ...
  if (!process.env.DARKIE_CODER_DIRECT_TRACE) {
    state = false
    return undefined
  }
