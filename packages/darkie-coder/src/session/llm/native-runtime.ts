...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("darkie-coder"))
    return { type: "unsupported", reason: "provider is not openai, darkie-coder, or anthropic" }
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains darkie-coder-owned. The native runtime only adapts
