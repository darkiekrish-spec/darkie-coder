...
import { Darkie CoderHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for darkie-coder serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(Darkie CoderHttpApi)
...
export const PublicApi = Darkie CoderHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "darkie-coder",
    version: "1.0.0",
    description: "darkie-coder api",
