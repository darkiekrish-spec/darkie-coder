...
const TUI_SCHEMA_URL = "https://darkie-coder.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from darkie-coder.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const darkie-coder = await darkie-coderFiles(input)
  for (const file of darkie-coder) {
...
async function darkie-coderFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "darkie-coder"),
    ...(await Filesystem.findUp(["darkie-coder.json", "darkie-coder.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "darkie-coder"))
  }
  if (Flag.DARKIE_CODER_CONFIG) files.push(Flag.DARKIE_CODER_CONFIG)
