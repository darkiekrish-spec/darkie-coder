...
export class Service extends Context.Service<Service, Interface>()("@darkie-coder/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old darkie-coder.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.darkie-coder`
  // folders between cwd and home, and DARKIE_CODER_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.DARKIE_CODER_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit DARKIE_CODER_TUI_CONFIG override, if set.
  if (Flag.DARKIE_CODER_TUI_CONFIG) {
    const configFile = Flag.DARKIE_CODER_TUI_CONFIG
  ...
  // 4. `.darkie-coder` directories (and DARKIE_CODER_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".darkie-coder") || dir === Flag.DARKIE_CODER_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".darkie-coder") && dir !== Flag.DARKIE_CODER_CONFIG_DIR) continue
