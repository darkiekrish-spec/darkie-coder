...
export class Service extends Context.Service<Service, Interface>()("@darkie-coder/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["darkie-coder.jsonc", "darkie-coder.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://darkie-coder.ai/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.DARKIE_CODER_CONFIG && !Flag.DARKIE_CODER_CONFIG_DIR && !Flag.DARKIE_CODER_CONFIG_CONTENT) {
        const file = globalConfigFile()
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://darkie-coder.ai/config.json" }, null, 2))
            .pipe(Effect.catch(() => Effect.void))
        }
      }
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "config.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "darkie-coder.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "darkie-coder.jsonc"), env))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://darkie-coder.ai/config.json"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "DARKIE_CODER_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/darkie-coder`
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://darkie-coder.ai/config.json"
        ...
        if (Flag.DARKIE_CODER_CONFIG) {
          yield* merge(Flag.DARKIE_CODER_CONFIG, yield* loadFile(Flag.DARKIE_CODER_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.DARKIE_CODER_CONFIG })
        }

        if (!Flag.DARKIE_CODER_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("darkie-coder", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.DARKIE_CODER_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from DARKIE_CODER_CONFIG_DIR", { path: Flag.DARKIE_CODER_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".darkie-coder") || dir === Flag.DARKIE_CODER_CONFIG_DIR) {
            for (const file of ["darkie-coder.json", "darkie-coder.jsonc"]) {
          ...
          // Auto-discovered plugins under `.darkie-coder/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.DARKIE_CODER_CONFIG_CONTENT) {
          const source = "DARKIE_CODER_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.DARKIE_CODER_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from DARKIE_CODER_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["DARKIE_CODER_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("DARKIE_CODER_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.DARKIE_CODER_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.DARKIE_CODER_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("DARKIE_CODER_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.DARKIE_CODER_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.DARKIE_CODER_DISABLE_PRUNE) {
