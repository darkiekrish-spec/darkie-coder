...
Delete `packages/darkie-coder/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/darkie-coder/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/darkie-coder/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/darkie-coder`
- TUI plugin loader tests in `packages/darkie-coder` if runtime plugin API wiring changed
