...
Practical reference for a future `packages/server` split after the darkie-coder
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/darkie-coder`.
...
Until enough shared service code lives outside `packages/darkie-coder`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/darkie-coder`

It should not import `packages/darkie-coder` services while `packages/darkie-coder`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on darkie-coder-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/darkie-coder` implementation details.
