...
Practical reference for the current tool-migration state in `packages/darkie-coder`.
...
Tool tests should use the existing Effect helpers in `packages/darkie-coder/test/lib/effect.ts`:
