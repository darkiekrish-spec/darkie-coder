...
Trim `packages/darkie-coder/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/darkie-coder/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/darkie-coder`.
- `[x]` `bun typecheck` from `packages/darkie-coder`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/darkie-coder/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/darkie-coder`.
- `[x]` `bun dev generate > /tmp/darkie-coder-openapi.json` from `packages/darkie-coder`.
...
- Edit `packages/darkie-coder/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/darkie-coder-openapi.json` from `packages/darkie-coder`.
...
- Check source schemas in files like `packages/darkie-coder/src/session/schema.ts`, `packages/darkie-coder/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/darkie-coder/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/darkie-coder/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
