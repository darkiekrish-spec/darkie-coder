...
```json
{
  "$schema": "https://darkie-coder.ai/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/darkie-coder-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- The value is a semver range checked against the running Darkie Coder version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.darkie-coder` is absent, no check is performed (backward compatible).
...
- `darkie-coder plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `darkie-coder plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.darkie-coder` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.darkie-coder`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.darkie-coder`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.darkie-coder` dir or `DARKIE_CODER_CONFIG_DIR`, Darkie Coder installs `@opencode-ai/plugin` into that dir and writes:
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Darkie Coder's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
Darkie Coder registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, Darkie Coder pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- `message` is required; `title` defaults to `"darkie-coder"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `darkie-coder.default`.
...
- Local plugins persist installed themes under the local `.darkie-coder/themes` area near the plugin config source.
...
- `--pure` / `DARKIE_CODER_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.darkie-coder/plugins/tui-smoke.tsx`
- Local vim plugin: `.darkie-coder/plugins/tui-vim.tsx`
- Local smoke config: `.darkie-coder/tui.json`
- Local smoke theme: `.darkie-coder/plugins/smoke-theme.json`
