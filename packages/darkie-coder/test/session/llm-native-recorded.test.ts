...
const zenURL = (connection: string) => `https://console.darkie-coder.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.DARKIE_CODER_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["darkie-coder", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("DARKIE_CODER_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("DARKIE_CODER_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["darkie-coder", "native", "oauth", "tool-loop"],
  ...
  {
    id: "darkie-coder-proxy",
    name: "Darkie Coder proxy",
    providerID: ProviderV2.ID.darkie-coder,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["darkie-coder", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.DARKIE_CODER_RECORD_CONSOLE_TOKEN && process.env.DARKIE_CODER_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.darkie-coder,
        name: "Darkie Coder Zen",
        env: ["DARKIE_CODER_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.DARKIE_CODER_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.DARKIE_CODER_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.DARKIE_CODER_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["darkie-coder", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("DARKIE_CODER_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("DARKIE_CODER_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("DARKIE_CODER_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set DARKIE_CODER_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the darkie-coder LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "darkie-coder.json"),
      JSON.stringify({ $schema: "https://darkie-coder.ai/config.json", ...scenario.config(model) }),
    ),
  )
