...
describe("session.llm.stream", () => {
  ...
  const darkie-coderFixture = { providerID: "darkie-coder-test", modelID: vivgridFixture.modelID }

  it.instance(
    "sends the parent session header for darkie-coder providers",
    () =>
      Effect.gen(function* () {
        ...
        const resolved = yield* Provider.use.getModel(
          ProviderV2.ID.make(darkie-coderFixture.providerID),
          ModelV2.ID.make(darkie-coderFixture.modelID),
        )
        ...
        const user = {
          ...
          agent: agent.name,
          model: {
            providerID: ProviderV2.ID.make(darkie-coderFixture.providerID),
            modelID: resolved.id,
          },
      ...
      }),
    {
      config: () => {
        const fixture = loadFixture(vivgridFixture.providerID, vivgridFixture.modelID)
        return {
          enabled_providers: [darkie-coderFixture.providerID],
          provider: {
            [darkie-coderFixture.providerID]: {
              name: "Darkie Coder Test",
