...
import type { Event, Darkie CoderClient } from "@opencode-ai/sdk/v2"
...
type PermissionReplyParams = Parameters<Darkie CoderClient["permission"]["reply"]>[0]
...
async function tempFile(name: string, content: string) {
  const dir = await mkdtemp(path.join(tmpdir(), "darkie-coder-acp-permission-"))
