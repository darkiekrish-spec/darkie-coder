...
const configLayer = TestConfig.layer({
  directories: () => InstanceState.directory.pipe(Effect.map((dir) => [path.join(dir, ".darkie-coder")])),
})
...
describe("tool.registry", () => {
  ...
  it.instance("loads tools from .darkie-coder/tool (singular)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const darkie-coder = path.join(test.directory, ".darkie-coder")
      const tool = path.join(darkie-coder, "tool")
  ...
  it.instance("ignores non-tool exports in .darkie-coder/tool files", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tool = path.join(test.directory, ".darkie-coder", "tool")
      yield* Effect.promise(() => fs.mkdir(tool, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  // came in through `plugin.list()` — `oh-my-darkie-coder` was registering a tool
  ...
  it.instance("loads tools from .darkie-coder/tools (plural)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const darkie-coder = path.join(test.directory, ".darkie-coder")
      const tools = path.join(darkie-coder, "tools")
  ...
  it.instance("loads Zod-schema custom tools with JSON Schema and validation", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const customTools = path.join(test.directory, ".darkie-coder", "tools")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const darkie-coder = path.join(test.directory, ".darkie-coder")
        const customTools = path.join(darkie-coder, "tools")
        const plugin = path.join(darkie-coder, "node_modules", "@opencode-ai", "plugin")
        yield* Effect.promise(() => fs.mkdir(path.join(plugin, "dist"), { recursive: true }))
        yield* Effect.promise(() => fs.mkdir(customTools, { recursive: true }))
        yield* Effect.promise(() =>
          fs.cp(path.dirname(fileURLToPath(import.meta.resolve("zod"))), path.join(darkie-coder, "node_modules", "zod"), {
            dereference: true,
            recursive: true,
          }),
        )
  ...
  it.instance("loads legacy JSON-schema-shaped custom tools with wire schema", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tools = path.join(test.directory, ".darkie-coder", "tools")
      yield* Effect.promise(() => fs.mkdir(tools, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools with external dependencies without crashing", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(darkie-coder, "package.json"),
          JSON.stringify({
            name: "custom-tools",
            dependencies: {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(darkie-coder, "package-lock.json"),
          JSON.stringify({
            name: "custom-tools",
            lockfileVersion: 3,
            packages: {
      ...
      const cowsay = path.join(darkie-coder, "node_modules", "cowsay")
