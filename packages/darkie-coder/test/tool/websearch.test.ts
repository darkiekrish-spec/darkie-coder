...
describe("websearch provider", () => {
  ...
  test("supports an operational override", () => {
    const original = process.env.DARKIE_CODER_WEBSEARCH_PROVIDER

    try {
      process.env.DARKIE_CODER_WEBSEARCH_PROVIDER = "parallel"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("parallel")

      process.env.DARKIE_CODER_WEBSEARCH_PROVIDER = "exa"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("exa")
    } finally {
      if (original === undefined) delete process.env.DARKIE_CODER_WEBSEARCH_PROVIDER
      else process.env.DARKIE_CODER_WEBSEARCH_PROVIDER = original
    }
  })
  ...
  test("is enabled for Darkie Coder providers or explicit websearch provider flags", () => {
    expect(webSearchEnabled(ProviderV2.ID.darkie-coder, { exa: false, parallel: false })).toBe(true)
    expect(webSearchEnabled(ProviderV2.ID.make("darkie-coder-go"), { exa: false, parallel: false })).toBe(true)
