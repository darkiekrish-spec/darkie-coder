    ...
    const msgs = [
      ...
      { role: "assistant", content: text("assistant string") },
      {
        ...
        content: [
          ...
          { type: "tool-call", toolCallId: "call-1", toolName: "Read", input: { filePath: ".darkie-coder/tool/emoji.ts" } },
          {
            type: "tool-result",
            toolCallId: "call-2",
            toolName: "Read",
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const darkie-coderModel = {
      ...openaiModel,
      providerID: "darkie-coder",
      api: {
        id: "darkie-coder-test",
        url: "https://api.darkie-coder.ai",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, darkie-coderModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.darkie-coder?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.darkie-coder?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          darkie-coder: { itemId: "msg_darkie-coder" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              darkie-coder: { itemId: "msg_darkie-coder_part" },
    ...
    expect(result[0].providerOptions?.darkie-coder?.itemId).toBe("msg_darkie-coder")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.darkie-coder?.itemId).toBe("msg_darkie-coder_part")
...
describe("ProviderTransform sampling defaults - DeepSeek", () => {
  ...
  test.each([
    ...
    ["darkie-coder", "deepseek-v4-flash"],
    ["darkie-coder-go", "deepseek-v4-flash"],
