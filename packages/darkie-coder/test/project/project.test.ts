...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const darkie-coderFile = path.join(tmp, ".git", "darkie-coder")
      expect(yield* Effect.promise(() => Bun.file(darkie-coderFile).exists())).toBe(false)
    }),
  )
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "darkie-coder")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "darkie-coder")
      const wrongCache = path.join(parentDir, ".git", "darkie-coder")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "darkie-coder")
      const cacheB = path.join(bareB, "darkie-coder")
      const wrongCache = path.join(parentDir, ".git", "darkie-coder")
