...
it.instance("plan agent denies edits except .darkie-coder/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".darkie-coder/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".darkie-coder", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.DARKIE_CODER_TEST_HOME
      process.env.DARKIE_CODER_TEST_HOME = test.directory
