...
export function createAcpClient(input: Pick<CliFixture, "darkie-coder">, env?: Record<string, string>) {
  return Effect.gen(function* () {
    return createJsonRpcAcpClient(yield* input.darkie-coder.acp(env ? { env } : undefined))
  })
}
  ...
  return Effect.gen(function* () {
    return expectOk(
      yield* acp.request<InitializeResponse>("initialize", {
        ...
        clientInfo: { name: "darkie-coder-local-acp", version: "0.1.0" },
      }),
    )
  })
}
