...
describe("darkie-coder acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ darkie-coder }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ darkie-coder }))
        ...
        expect(initialized.agentInfo?.name).toBe("Darkie Coder")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ darkie-coder }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ darkie-coder })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("darkie-coder-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "darkie-coder-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.DARKIE_CODER_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ darkie-coder }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("darkie-coder-login")
