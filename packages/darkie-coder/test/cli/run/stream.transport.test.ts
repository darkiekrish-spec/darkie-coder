...
import { Darkie CoderClient, type GlobalEvent } from "@opencode-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<Darkie CoderClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<Darkie CoderClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<Darkie CoderClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<Darkie CoderClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<Darkie CoderClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://darkie-coder.test"),
...
function sdk(
  input: {
    ...
    subscribe?: Darkie CoderClient["event"]["subscribe"]
    globalEvent?: Darkie CoderClient["global"]["event"]
    promptAsync?: Darkie CoderClient["session"]["promptAsync"]
    status?: Darkie CoderClient["session"]["status"]
    messages?: Darkie CoderClient["session"]["messages"]
    children?: Darkie CoderClient["session"]["children"]
    permissions?: Darkie CoderClient["permission"]["list"]
    questions?: Darkie CoderClient["question"]["list"]
  } = {},
) {
  const client = new Darkie CoderClient()

  const subscribe: Darkie CoderClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: Darkie CoderClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: Darkie CoderClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: Darkie CoderClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: Darkie CoderClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: Darkie CoderClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: Darkie CoderClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: Darkie CoderClient["question"]["list"] = input.questions ?? (() => ok([]))
