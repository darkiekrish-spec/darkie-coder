// Subprocess integration tests for `darkie-coder run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `darkie-coder.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `DARKIE_CODER_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("darkie-coder run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, darkie-coder }) =>
      Effect.gen(function* () {
        ...
        const result = yield* darkie-coder.run("say hi")
        darkie-coder.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, darkie-coder }) =>
      Effect.gen(function* () {
        ...
        const result = yield* darkie-coder.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, darkie-coder }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* darkie-coder.run("think", { extraArgs: ["--thinking"] })
        darkie-coder.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* darkie-coder.run("think again")
        darkie-coder.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, darkie-coder }) =>
      Effect.gen(function* () {
        ...
        const result = yield* darkie-coder.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, darkie-coder }) =>
      Effect.gen(function* () {
        ...
        const result = yield* darkie-coder.run("say hi", { format: "json" })
        darkie-coder.expectExit(result, 0)

        const events = darkie-coder.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ darkie-coder }) =>
      Effect.gen(function* () {
        const result = yield* darkie-coder.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, darkie-coder }) =>
      Effect.gen(function* () {
        ...
        const result = yield* darkie-coder.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, darkie-coder }) =>
      Effect.gen(function* () {
        ...
        yield* llm.text("recovered")
        const result = yield* darkie-coder.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, darkie-coder }) =>
      Effect.gen(function* () {
        yield* llm.tool("bash", { command: "rm -f denied-file", description: "Remove a test file" })
        yield* llm.text("continued after rejection")
        const denied = yield* darkie-coder.run("request permission", { permission: { bash: "ask" } })
        darkie-coder.expectExit(denied, 0)
        ...
        const allowed = yield* darkie-coder.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        darkie-coder.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* darkie-coder.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        darkie-coder.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, darkie-coder }) =>
      Effect.gen(function* () {
        ...
        const server = yield* darkie-coder.serve()

        const result = yield* darkie-coder.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, darkie-coder }) =>
      Effect.gen(function* () {
        const result = yield* darkie-coder.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, darkie-coder }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* darkie-coder.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
