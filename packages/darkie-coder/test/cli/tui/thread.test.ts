...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ darkie-coder }) =>
    Effect.gen(function* () {
      const result = yield* darkie-coder.spawn(["--replay-limit", "10"])

      darkie-coder.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ darkie-coder }) =>
    Effect.gen(function* () {
      const result = yield* darkie-coder.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ darkie-coder }) =>
    Effect.gen(function* () {
      const result = yield* darkie-coder.spawn(["--mini", "--port", "4096"])
