...
describe("darkie-coder read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // DARKIE_CODER_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ darkie-coder }) =>
      Effect.gen(function* () {
        const r = yield* darkie-coder.spawn(["mcp", "list"])
        darkie-coder.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ darkie-coder }) =>
      Effect.gen(function* () {
        const r = yield* darkie-coder.spawn(["providers", "list"])
        darkie-coder.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ darkie-coder }) =>
      Effect.gen(function* () {
        const r = yield* darkie-coder.spawn(["models"])
        darkie-coder.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ darkie-coder }) =>
      Effect.gen(function* () {
        const r = yield* darkie-coder.spawn(["agent", "list"])
        darkie-coder.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh DARKIE_CODER_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ darkie-coder }) =>
      Effect.gen(function* () {
        const r = yield* darkie-coder.spawn(["session", "list"])
        darkie-coder.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ darkie-coder }) =>
      Effect.gen(function* () {
        const r = yield* darkie-coder.spawn(["stats"])
        darkie-coder.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under DARKIE_CODER_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ darkie-coder }) =>
      Effect.gen(function* () {
        const r = yield* darkie-coder.spawn(["db", "path"])
        darkie-coder.expectExit(r, 0, "db path")
