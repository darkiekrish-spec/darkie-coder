...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/darkie-coder.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/darkie-coder.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/darkie-coder.jsonc", dir: ".darkie-coder", suggestion: "darkie-coder" },
        expected:
          'Directory ".darkie-coder" in /tmp/darkie-coder.jsonc is not valid. Rename the directory to "darkie-coder" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        data: {
          path: "/tmp/darkie-coder.jsonc",
          message: "schema mismatch",
          issues: [{ message: "Expected string", path: ["provider", "id"] }],
        },
        expected: "Configuration is invalid at /tmp/darkie-coder.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
  ...
  test("formats account transport errors clearly", () => {
    const error = new AccountTransportError({
      method: "POST",
      url: "https://console.darkie-coder.ai/auth/device/code",
    })

    const formatted = FormatError(error)

    expect(formatted).toContain("Could not reach POST https://console.darkie-coder.ai/auth/device/code.")
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Try: `darkie-coder models` to list available models",
      "Or check your config (darkie-coder.json) provider/model names",
    ].join("\n")
