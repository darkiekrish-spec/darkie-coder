...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-darkie-coder-directory", directory)
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-darkie-coder-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://darkie-coder.ai/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-darkie-coder-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-darkie-coder-workspace")
