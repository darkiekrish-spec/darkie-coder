...
describe("HttpApi workspace proxy", () => {
  ...
  it.live("strips darkie-coder-internal headers and merges extra headers", () =>
    Effect.gen(function* () {
      ...
      const request = HttpServerRequest.fromWeb(
        new Request("http://localhost/test", {
          headers: {
            ...
            "x-darkie-coder-workspace": "ws_123",
            "x-custom": "preserved",
          },
        }),
      )
      ...
      expect(forwarded["x-darkie-coder-directory"]).toBeUndefined()
      expect(forwarded["x-darkie-coder-workspace"]).toBeUndefined()
