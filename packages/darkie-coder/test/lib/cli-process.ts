// Subprocess test harness for the darkie-coder CLI. Spawns the real binary against
...
//
// Configuration flows through darkie-coder's built-in test affordances:
//   - DARKIE_CODER_CONFIG_CONTENT      : provider config inline, no files to find
//   - DARKIE_CODER_TEST_HOME           : pins os.homedir() → tmpdir
//   - DARKIE_CODER_DISABLE_PROJECT_CONFIG : skip walking up for darkie-coder.json
//   - DARKIE_CODER_PURE                : skip external plugin discovery + install
//   - DARKIE_CODER_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `darkie-coder.run` is fully wired. The shape supports adding more
// builders (`darkie-coder.serve(opts)`, `darkie-coder.acp(opts)`, `darkie-coder.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of Darkie CoderCli.
...
const darkie-coderRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(darkie-coderRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    DARKIE_CODER_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    DARKIE_CODER_CONFIG_CONTENT: configJson,
    DARKIE_CODER_DISABLE_PROJECT_CONFIG: "1",
    DARKIE_CODER_PURE: "1",
    DARKIE_CODER_DISABLE_AUTOUPDATE: "1",
    DARKIE_CODER_DISABLE_AUTOCOMPACT: "1",
    DARKIE_CODER_DISABLE_MODELS_FETCH: "1",
    DARKIE_CODER_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `darkie-coder run`. New flags should
...
// `darkie-coder serve` is a long-lived process — it never exits on its own.
...
// `darkie-coder acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type Darkie CoderCli = {
  ...
  // Spawn `darkie-coder serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `darkie-coder acp` and return a duplex JSON-RPC handle. Long-lived:
...
export type CliFixture = {
  ...
  readonly darkie-coder: Darkie CoderCli
}
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("darkie-coder.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          DARKIE_CODER_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("darkie-coder.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("darkie-coder.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "darkie-coder server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `darkie-coder serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("darkie-coder.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const darkie-coder: Darkie CoderCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, darkie-coder })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `darkie-coder.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "darkie-coder") {
...
// every call site. Use this for any test that needs the darkie-coder CLI fixture.
//
...
// resources (e.g. `darkie-coder.serve`) without an extra `Effect.scoped` wrapper —
