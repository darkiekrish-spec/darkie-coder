...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "darkie-coder.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "darkie-coder.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "darkie-coder.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "darkie-coder.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["darkie-coder.json", "darkie-coder.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "darkie-coder.json"),
        path.join(project, "darkie-coder.jsonc"),
      ])
    })
  })
