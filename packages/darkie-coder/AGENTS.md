...
- Running `bun dev` from `packages/darkie-coder` starts the live interactive TUI. Do not run it as a blocking foreground command when you need to inspect the result.
- Start it in `tmux` instead: `tmux new-session -d -s darkie-coder-dev 'bun dev'`.
- Capture the current TUI output with: `tmux capture-pane -pt darkie-coder-dev`.
- Stop the session explicitly when done: `tmux kill-session -t darkie-coder-dev`.
...
export class Service extends Context.Service<Service, Interface>()("@darkie-coder/Foo") {}
