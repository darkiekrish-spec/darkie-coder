...
This is the living design and status document for `@opencode-ai/codemode` and its existing V2 Darkie Coder adapter.
...
### V2 Darkie Coder adapter

This section describes the `v2` branch integration. On `dev`, CodeMode is integrated through
`packages/darkie-coder/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
