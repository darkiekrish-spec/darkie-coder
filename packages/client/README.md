...
Private generation target for clients derived directly from Darkie Coder's authoritative Effect `HttpApi`.
...
```ts
import { AbsolutePath, Location, Darkie Coder, Prompt } from "@opencode-ai/client/effect"

const client = yield * Darkie Coder.make({ baseUrl: "https://darkie-coder.example" })
yield *
