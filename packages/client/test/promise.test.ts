...
import { isSessionNotFoundError, isUnauthorizedError, Darkie Coder } from "../src"

test("exposes every standard HTTP API group", () => {
  const client = Darkie Coder.make({ baseUrl: "http://localhost:3000" })
...
test("sessions.get returns the wire projection", async () => {
  const client = Darkie Coder.make({
