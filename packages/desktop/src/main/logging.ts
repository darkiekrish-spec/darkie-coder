...
export async function exportDebugLogs() {
  ...
  const output = join(app.getPath("downloads"), `darkie-coder-debug-${stamp()}.zip`)
  try {
...
function serverLogRoots() {
  ...
  return [...new Set([join(xdgData, "darkie-coder", "log"), join(app.getPath("userData"), "darkie-coder", "log")])]
}
