...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.darkie-coder.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `darkie-coder.settings.dat` is special: it maps to the `darkie-coder.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"darkie-coder.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // darkie-coder.settings.dat → the electron settings store ("darkie-coder.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "darkie-coder.global.dat").
  const storeName = filename === "darkie-coder.settings.dat" ? "darkie-coder.settings" : filename
