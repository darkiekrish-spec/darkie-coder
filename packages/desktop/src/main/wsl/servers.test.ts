...
let releaseDarkie CoderResolve: (() => void) | undefined
...
test("rejects an update that did not install the desktop version", () => {
  expect(() => expectDarkie CoderVersion("1.16.2", "1.16.2")).not.toThrow()
  expect(() => expectDarkie CoderVersion("1.14.35", "1.16.2")).toThrow(
    "Darkie Coder update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating Darkie Coder", () => {
  expect(
    wslServerIdToRestart(
      [
        {
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.darkie-coder/bin/darkie-coder",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, darkie-coderChecks: {} })
})
...
test("ignores stale background Darkie Coder checks after removing a WSL server", async () => {
  persistedServers = []
  releaseDarkie CoderResolve = undefined
  ...
  await waitFor(() => !!releaseDarkie CoderResolve)
  await controller.removeServer("wsl:Debian")
  releaseDarkie CoderResolve?.()
  await new Promise((resolve) => setTimeout(resolve, 0))

  expect(controller.getState().servers).toEqual([])
  expect(controller.getState().darkie-coderChecks).toEqual({})
})

test("ignores stale startup Darkie Coder checks after removing a WSL server", async () => {
...
test("probes addable distros in parallel before checking Darkie Coder", async () => {
  persistedServers = []
  const started: string[] = []
  const release = new Map<string, () => void>()
  const darkie-coder: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...
    resolveDarkie Coder: async (distro) => {
      darkie-coder.push(distro)
      return "/home/me/.darkie-coder/bin/darkie-coder"
    },
  })
  ...
  expect(darkie-coder).toEqual([])
  release.get("Debian")?.()
  release.get("Ubuntu")?.()
  await task

  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(darkie-coder).toEqual(["Debian", "Ubuntu"])
  expect(Object.keys(controller.getState().darkie-coderChecks)).toEqual(["Debian", "Ubuntu"])
})

test("does not check Darkie Coder in addable distros that cannot execute commands", async () => {
  persistedServers = []
  const darkie-coder: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),
  ...
  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(darkie-coder).toEqual(["Debian"])
  expect(Object.keys(controller.getState().darkie-coderChecks)).toEqual(["Debian"])
})
...
function testControllerOptions() {
  return {
    ...
    resolveDarkie Coder: async () => {
      await new Promise<void>((resolve) => {
        releaseDarkie CoderResolve = resolve
      })
      return "/home/me/.darkie-coder/bin/darkie-coder"
    },
  }
