...
import { type WslCommandLine, resolveWslDarkie Coder, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const darkie-coder = await resolveWslDarkie Coder(distro)
  if (!darkie-coder) throw new Error(nativeT("desktop.wsl.error.darkie-coderNotInstalled", { distro }))

  const port = await allocatePort()
  const password = randomUUID()
  const username = "darkie-coder"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export DARKIE_CODER_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export DARKIE_CODER_CLIENT=desktop",
    `export DARKIE_CODER_SERVER_USERNAME=${shellEscape(username)}`,
    `export DARKIE_CODER_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(darkie-coder)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
