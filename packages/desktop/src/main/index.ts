...
const APP_NAMES: Record<string, string> = {
  dev: "Darkie Coder Dev",
  beta: "Darkie Coder Beta",
  prod: "Darkie Coder",
}
...
const TEST_ONBOARDING = process.env.DARKIE_CODER_TEST_ONBOARDING === "1"
const SIDECAR_VERSION = process.env.DARKIE_CODER_SIDECAR_V2 === "1" ? "v2" : "v1"
...
const main = Effect.gen(function* () {
  ...
  process.env.DARKIE_CODER_DISABLE_EMBEDDED_WEB_UI = "true"

  const appId = app.isPackaged ? APP_IDS[CHANNEL] : "ai.darkie-coder.desktop.dev"
  const onboardingTestRoot = ((): string | undefined => {
    if (!TEST_ONBOARDING) return

    const root = join(tmpdir(), `darkie-coder-onboarding-${randomUUID()}`)
  ...
  app.setName(app.isPackaged ? APP_NAMES[CHANNEL] : "Darkie Coder Dev")
  ...
  app.on("second-instance", (_event: Event, argv: string[]) => {
    const urls = argv.filter((arg: string) => arg.startsWith("darkie-coder://"))
    if (urls.length) {
  ...
  app.setAsDefaultProtocolClient("darkie-coder")
  registerRendererProtocol()
  setDockIcon()
  ...
  const loadingTask = yield* Effect.gen(function* () {
    ...
    const port = yield* Effect.gen(function* () {
      const fromEnv = process.env.DARKIE_CODER_PORT
      if (fromEnv) {
