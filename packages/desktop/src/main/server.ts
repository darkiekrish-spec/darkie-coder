...
const SIDECAR_SERVICE_NAME = "darkie-coder server"
...
export function preferAppEnv(userDataPath: string) {
  ...
  Object.assign(process.env, {
    ...shellEnv,
    DARKIE_CODER_EXPERIMENTAL_ICON_DISCOVERY: "true",
...
export async function checkHealth(url: string, password?: string | null): Promise<boolean> {
  ...
  if (password) {
    const auth = Buffer.from(`darkie-coder:${password}`).toString("base64")
