...
describe("hasExistingAppState", () => {
  ...
  test("recognizes state written by an earlier Darkie Coder launch", () => {
    expect(hasExistingAppState([file("darkie-coder.settings")])).toBe(true)
    expect(hasExistingAppState([file("darkie-coder.global.dat")])).toBe(true)
    expect(hasExistingAppState([file("window-state-abc.json")])).toBe(true)
    expect(hasExistingAppState([directory("darkie-coder")])).toBe(true)
  })
})
