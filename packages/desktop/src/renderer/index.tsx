    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.DARKIE_CODER_CHANNEL === "prod" &&
...
const deepLinkEvent = "darkie-coder:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__DARKIE_CODER__ ??= {}
  const pending = window.__DARKIE_CODER__.deepLinks ?? []
  window.__DARKIE_CODER__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `darkie-coder.desktop.window.${windowID}.last-active-url`
}
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("darkie-coder.global.dat").getItem("language")
