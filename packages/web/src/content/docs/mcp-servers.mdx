...
You can add external tools to Darkie Coder using the _Model Context Protocol_, or MCP. Darkie Coder supports both local and remote servers.
...
You can define MCP servers in your [Darkie Coder Config](https://darkie-coder.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/darkie-coder` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Darkie Coder automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Darkie Coder will:
...
If the server requires authentication, Darkie Coder will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `darkie-coder mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Darkie Coder will store the tokens securely in `~/.local/share/darkie-coder/mcp-auth.json`.
...
Your MCPs are available as tools in Darkie Coder, alongside built-in tools. So you can manage them through the Darkie Coder config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Darkie Coder to your Sentry account.
