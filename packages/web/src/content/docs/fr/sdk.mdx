---
title: SDK
description: Client JS de type sécurisé pour le serveur darkie-coder.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK darkie-coder JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler darkie-coder par programme.
...
Créez une instance de darkie-coder :

```javascript
import { createDarkie Coder } from "@opencode-ai/sdk"

const { client } = await createDarkie Coder()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `darkie-coder.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${darkie-coder.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de darkie-coder, vous pouvez créer une instance client pour vous y connecter :
...
const textResults = await client.find.text({
  query: { pattern: "function.*darkie-coder" },
})
