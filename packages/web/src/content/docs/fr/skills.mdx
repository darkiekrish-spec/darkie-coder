...
Les compétences d'agent permettent à Darkie Coder de découvrir des instructions réutilisables à partir de votre dépôt ou de votre répertoire personnel.
...
Darkie Coder recherche ces emplacements :

- Configuration du projet : `.darkie-coder/skills/<name>/SKILL.md`
- Configuration globale : `~/.config/darkie-coder/skills/<name>/SKILL.md`
...
Pour les chemins locaux du projet, Darkie Coder remonte de votre répertoire de travail actuel jusqu'à ce qu'il atteigne l'arbre de travail git.
Il charge tout `skills/*/SKILL.md` correspondant dans `.darkie-coder/` et tout `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` correspondant en cours de route.

Les définitions globales sont également chargées à partir de `~/.config/darkie-coder/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` et `~/.agents/skills/*/SKILL.md`.
...
Créez `.darkie-coder/skills/git-release/SKILL.md` comme ceci :
...
Darkie Coder répertorie les compétences disponibles dans la description de l'outil `skill`.
...
Contrôlez les compétences auxquelles les agents peuvent accéder à l'aide d'autorisations basées sur des modèles dans `darkie-coder.json` :
...
**Pour les agents intégrés** (dans `darkie-coder.json`) :
