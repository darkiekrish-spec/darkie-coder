...
Avec Darkie Coder, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Darkie Coder utilise notre propre thème `darkie-coder`.
...
Darkie Coder est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Darkie Coder corresponde à l'apparence de leur terminal
...
Darkie Coder prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/darkie-coder/themes/*.json` ou `$XDG_CONFIG_HOME/darkie-coder/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.darkie-coder/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.darkie-coder/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/darkie-coder/themes
vim ~/.config/darkie-coder/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .darkie-coder/themes
vim .darkie-coder/themes/my-theme.json
```
...
{
  "$schema": "https://darkie-coder.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
