...
Pour déboguer les problèmes avec Darkie Coder, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\darkie-coder\log`
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `darkie-coder --log-level DEBUG`.

---

## Stockage

darkie-coder stocke les données de session et autres données d'application sur le disque à l'emplacement :

- **macOS/Linux** : `~/.local/share/darkie-coder/`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\darkie-coder`
...
Darkie Coder Desktop exécute un serveur Darkie Coder local (le side-car `darkie-coder-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `Darkie Coder` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** (anciennes installations) : `~/.local/share/darkie-coder/darkie-coder.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\darkie-coder\darkie-coder.jsonc`
...
Darkie Coder peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :

- **Plugins mondiaux**
  - **macOS/Linux** : `~/.config/darkie-coder/plugins/`
  - **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\darkie-coder\plugins`
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que Darkie Coder puisse le reconstruire.

1. Quittez complètement Darkie Coder Desktop.
2. Supprimez le répertoire cache :

- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/darkie-coder`
- **Linux** : supprimez `~/.cache/darkie-coder` (ou exécutez `rm -rf ~/.cache/darkie-coder`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\darkie-coder`

3. Redémarrez le bureau Darkie Coder.

---

### Résoudre les problèmes de connexion au serveur

Darkie Coder Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `darkie-coder.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.

#### Vérifier les variables d'environnement

Si `DARKIE_CODER_PORT` est défini dans votre environnement, l'application de bureau tentera d'utiliser ce port pour le serveur local.

- Désactivez `DARKIE_CODER_PORT` (ou choisissez un port libre) et redémarrez.
...
Sur Windows, Darkie Coder Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de Darkie Coder.

---

### Les notifications ne s'affichent pas

Darkie Coder Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour Darkie Coder dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau Darkie Coder.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application Darkie Coder Desktop) :

- `darkie-coder.settings.dat` (serveur de bureau par défaut URL)
- `darkie-coder.global.dat` et `darkie-coder.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
Si vous rencontrez des problèmes avec Darkie Coder :
...
3. Assurez-vous d'avoir la dernière version avec `darkie-coder upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `darkie-coder models`
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.local\share\darkie-coder`
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. darkie-coder installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.cache\darkie-coder`

2. Redémarrez darkie-coder pour réinstaller les derniers packages du fournisseur

Cela forcera darkie-coder à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
darkie-coder détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.
