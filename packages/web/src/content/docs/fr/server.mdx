---
title: Serveur
description: Interagissez avec le serveur darkie-coder via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `darkie-coder serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client darkie-coder peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `darkie-coder.local` |
...
Définissez `DARKIE_CODER_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `darkie-coder`, ou définissez `DARKIE_CODER_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `darkie-coder serve` et à `darkie-coder web`.
...
Lorsque vous exécutez `darkie-coder`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur darkie-coder pour interagir avec darkie-coder par programmation.
:::

Cette architecture permet à darkie-coder de prendre en charge plusieurs clients et vous permet d'interagir avec darkie-coder par programme.

Vous pouvez exécuter `darkie-coder serve` pour démarrer un serveur autonome. Si vous avez le
darkie-coder TUI en cours d'exécution, `darkie-coder serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Darkie Coder [IDE](/docs/ide).
...
Le serveur darkie-coder expose les API suivantes.
