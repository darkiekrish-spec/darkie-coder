---
title: Plugins
description: Écrivez vos propres plugins pour étendre Darkie Coder.
---

Les plugins vous permettent d'étendre Darkie Coder en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Darkie Coder.
...
- `.darkie-coder/plugins/` - Plugins au niveau du projet
- `~/.config/darkie-coder/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/darkie-coder/node_modules/`.
...
1. Configuration globale (`~/.config/darkie-coder/darkie-coder.json`)
2. Configuration du projet (`darkie-coder.json`)
3. Répertoire global des plugins (`~/.config/darkie-coder/plugins/`)
4. Répertoire des plugins du projet (`.darkie-coder/plugins/`)
...
Darkie Coder exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK darkie-coder pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre darkie-coder.
...
:::note
Si vous utilisez l'application de bureau Darkie Coder, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez darkie-coder de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à darkie-coder :
...
L'assistant `tool` crée un outil personnalisé que darkie-coder peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour darkie-coder aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://darkie-coder.ai/docs/sdk) pour plus de détails.
