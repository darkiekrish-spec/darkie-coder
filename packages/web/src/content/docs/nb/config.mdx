---
title: Konfigurasjon
description: Ved å bruke darkie-coder JSON-konfigurasjonen.
---

Du kan konfigurere Darkie Coder ved å bruke en JSON konfigurasjonsfil.

---

## Format

Darkie Coder støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
For eksempel, hvis din globale konfigurasjon setter `theme: "darkie-coder"` og `autoupdate: true`, og prosjektkonfigurasjonen setter `model: "anthropic/claude-sonnet-4-5"`, vil den endelige konfigurasjonen inkludere alle tre innstillingene.
...
1. **Ekstern konfigurasjon** (fra `.well-known/darkie-coder`) - organisasjonsstandarder
2. **Global konfigurasjon** (`~/.config/darkie-coder/darkie-coder.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`DARKIE_CODER_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`darkie-coder.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.darkie-coder` kataloger** - agenter, kommandoer, plugins
6. **Inline config** (`DARKIE_CODER_CONFIG_CONTENT` env var) - kjøretidsoverstyringer
...
:::note
`.darkie-coder` og `~/.config/darkie-coder` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Organisasjoner kan tilby standardkonfigurasjon via `.well-known/darkie-coder`-endepunktet. Dette hentes automatisk når du autentiserer med en leverandør som støtter det.
...
Plasser din globale Darkie Coder-konfigurasjon i `~/.config/darkie-coder/darkie-coder.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `darkie-coder.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.

:::tip
Plasser prosjektspesifikk konfigurasjon i roten til prosjektet ditt.
:::

Når Darkie Coder starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
Spesifiser en tilpasset konfigurasjonsfilbane ved å bruke miljøvariabelen `DARKIE_CODER_CONFIG`.
...
Spesifiser en tilpasset konfigurasjonskatalog ved å bruke `DARKIE_CODER_CONFIG_DIR`
miljøvariabel. Denne katalogen vil bli søkt etter agenter, kommandoer,
moduser og plugins akkurat som standard `.darkie-coder` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.darkie-coder`-katalogene, slik at den **kan overstyre** innstillingene deres.

---

## Skjema

Konfigurasjonsfilen har et skjema som er definert i [**`darkie-coder.ai/config.json`**](https://darkie-coder.ai/config.json).
...
Du kan konfigurere serverinnstillinger for kommandoene `darkie-coder serve` og `darkie-coder web` gjennom alternativet `server`.
...
- `mdns` - Aktiver mDNS-tjenesteoppdagelse. Dette lar andre enheter på nettverket oppdage Darkie Coder-serveren din.
- `mdnsDomain` - Egendefinert domenenavn for mDNS-tjeneste. Standard er `darkie-coder.local`. Nyttig for å kjøre flere forekomster på samme nettverk.
...
Du kan konfigurere leverandørene og modellene du vil bruke i Darkie Coder-konfigurasjonen gjennom alternativene `provider`, `model` og `small_model`.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver Darkie Coder å bruke en rimeligere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/darkie-coder/agents/` eller `.darkie-coder/agents/`. [Les mer her](/docs/agents).
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil Darkie Coder falle tilbake til `"build"` med en advarsel.

Denne innstillingen gjelder for alle grensesnitt: TUI, CLI (`darkie-coder run`), skrivebordsapp og GitHub Action.
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/darkie-coder/commands/` eller `.darkie-coder/commands/`. [Les mer her](/docs/commands).
...
Darkie Coder vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater Darkie Coder alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider Darkie Coder med tilpassede verktøy, kroker og integrasjoner.

Plasser plugin-filer i `.darkie-coder/plugins/` eller `~/.config/darkie-coder/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
...
Dette er nyttig når du vil begrense Darkie Coder til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.
