...
Du kan legge til eksterne verktøy i Darkie Coder ved å bruke _Model Context Protocol_ eller MCP. Darkie Coder støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Darkie Coder Config](https://darkie-coder.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="darkie-coder.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/darkie-coder`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```jsonc title="darkie-coder.jsonc" {15}
{
  "$schema": "https://darkie-coder.ai/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
Darkie Coder håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Darkie Coder:
...
Hvis serveren krever autentisering, vil Darkie Coder be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `darkie-coder mcp auth <server-name>`.
...
```json title="darkie-coder.json" {7-11}
{
  "$schema": "https://darkie-coder.ai/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
darkie-coder mcp auth my-oauth-server
```
...
```bash
darkie-coder mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Darkie Coder lagre tokens sikkert i `~/.local/share/darkie-coder/mcp-auth.json`.
...
```json title="darkie-coder.json" {7}
{
  "$schema": "https://darkie-coder.ai/config.json",
  "mcp": {
    "my-api-key-server": {
...
darkie-coder mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Darkie Coder, sammen med innebygde verktøy. Så du kan administrere dem gjennom Darkie Coder-konfigurasjonen som alle andre verktøy.
...
```json title="darkie-coder.json" {14}
{
  "$schema": "https://darkie-coder.ai/config.json",
  "mcp": {
    "my-mcp-foo": {
...
```json title="darkie-coder.json" {11, 14-18}
{
  "$schema": "https://darkie-coder.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```json title="darkie-coder.json" {4-8}
{
  "$schema": "https://darkie-coder.ai/config.json",
  "mcp": {
    "sentry": {
...
```bash
darkie-coder mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Darkie Coder til Sentry-kontoen din.
...
```json title="darkie-coder.json" {4-7}
{
  "$schema": "https://darkie-coder.ai/config.json",
  "mcp": {
    "context7": {
...
```json title="darkie-coder.json" {7-9}
{
  "$schema": "https://darkie-coder.ai/config.json",
  "mcp": {
    "context7": {
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Darkie Coder-samtale oppført som en separat økt.
