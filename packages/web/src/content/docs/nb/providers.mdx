---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Darkie Coder.
---

import config from "../../../../config.mjs"
export const console = config.console

Darkie Coder bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Darkie Coder-konfigurasjonen.
...
i `~/.local/share/darkie-coder/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Darkie Coder
konfig.
...
Darkie Coder Zen er en liste over modeller levert av Darkie Coder-teamet som har vært
testet og verifisert for å fungere godt med Darkie Coder. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Darkie Coder Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Darkie Coder Zen` og gå til [darkie-coder.ai/auth](https://darkie-coder.ai/zen).
...
Det fungerer som alle andre leverandører i Darkie Coder og er helt valgfritt å bruke.

---

## Darkie Coder Go

Darkie Coder Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Darkie Coder-teamet som har vært
testet og verifisert for å fungere godt med Darkie Coder.

1. Kjør kommandoen `/connect` i TUI, velg `Darkie Coder Go`, og gå til [darkie-coder.ai/auth](https://darkie-coder.ai/zen).
...
Slik bruker du Amazon Bedrock med Darkie Coder:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Darkie Coder:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `darkie-coder.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i darkie-coder støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere darkie-coder til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Darkie Coder skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Darkie Coder-konfigurasjonen.
...
for å lagre token i Darkie Coder auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Darkie Coder bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Darkie Coder
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`darkie-coder.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [darkie-coder-gitlab-auth](https://www.npmjs.com/package/darkie-coder-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `darkie-coder.json`:
...
Slik bruker du GitHub Copilot-abonnementet med Darkie Coder:
...
Slik bruker du Google Vertex AI med Darkie Coder:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Darkie Coder.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Darkie Coder, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [darkie-coder-helicone-session](https://github.com/H2Shami/darkie-coder-helicone-session) plugin for automatisk å logge hver Darkie Coder-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Darkie Coder-samtale oppført som en separat økt.
...
Du kan konfigurere Darkie Coder for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Darkie Coder for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Darkie Coder for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Darkie Coder. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/darkie-coder) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Darkie Coder:
...
3. Kopier API-nøkkelen for bruk i Darkie Coder.
...
6. **Viktig**: Før du bruker skymodeller i Darkie Coder, må du hente modellinformasjonen lokalt:
...
Darkie Coder Zen er en liste over testede og verifiserte modeller levert av Darkie Coder-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Darkie Coder Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Darkie Coder Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Darkie Coder API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Darkie Coder-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Darkie Coder-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Darkie Coder:
...
Du kan også tilpasse modeller gjennom Darkie Coder-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Darkie Coder. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `darkie-coder.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Darkie Coder forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `darkie-coder auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Darkie Coder-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Darkie Coder-konfigurasjonen.
