---
title: Tjener
description: Samhandle med Darkie Coder-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `darkie-coder serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Darkie Coder-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `darkie-coder.local` |
...
Sett `DARKIE_CODER_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `darkie-coder` som standard, eller sett `DARKIE_CODER_SERVER_USERNAME` for å overstyre det. Dette gjelder både `darkie-coder serve` og `darkie-coder web`.
...
Når du kjører `darkie-coder` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Darkie Coder-serveren til å samhandle med Darkie Coder programmatisk.
:::

Denne arkitekturen lar Darkie Coder støtte flere klienter og lar deg samhandle med Darkie Coder programmatisk.

Du kan kjøre `darkie-coder serve` for å starte en frittstående server. Hvis du har
Darkie Coder TUI kjører, vil `darkie-coder serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Darkie Coder [IDE](/docs/ide) plugins.
...
darkie-coder-serveren viser følgende APIer.
