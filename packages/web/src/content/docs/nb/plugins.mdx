...
description: Skriv dine egne programtillegg for å utvide Darkie Coder.
---

Plugins lar deg utvide Darkie Coder ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Darkie Coder.
...
- `.darkie-coder/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/darkie-coder/node_modules/`.
...
1. Global konfigurasjon (`~/.config/darkie-coder/darkie-coder.json`)
2. Prosjektkonfigurasjon (`darkie-coder.json`)
3. Global plugin-katalog (`~/.config/darkie-coder/plugins/`)
4. Prosjektpluginkatalog (`.darkie-coder/plugins/`)
...
Darkie Coder kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Darkie Coder SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Darkie Coder.
...
:::note
Hvis du bruker Darkie Coder-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Darkie Coder fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Darkie Coder:
...
`tool`-hjelperen lager et tilpasset verktøy som Darkie Coder kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Darkie Coder sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://darkie-coder.ai/docs/sdk) for detaljer.
