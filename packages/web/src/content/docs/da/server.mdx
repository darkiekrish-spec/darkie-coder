---
title: Server
description: Interager med darkie-coder-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `darkie-coder serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en darkie-coder-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `darkie-coder.local` |
...
Indstil `DARKIE_CODER_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `darkie-coder`, eller indstil `DARKIE_CODER_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `darkie-coder serve` og `darkie-coder web`.
...
Når du kører `darkie-coder` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug darkie-coder-serveren til at interagere med darkie-coder programmatisk.
:::

Denne arkitektur lader darkie-coder understøtte flere klienter og lader dig interagere med darkie-coder programmatisk.

Du kan køre `darkie-coder serve` for at starte en selvstændig server. Hvis du har
darkie-coder TUI kørende, vil `darkie-coder serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Darkie Coder [IDE](/docs/ide) plugins.
...
Darkie Coder-serveren viser følgende API'er.
