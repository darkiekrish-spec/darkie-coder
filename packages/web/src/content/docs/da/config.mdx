---
title: Konfiguration
description: Ved at bruge Darkie Coder JSON konfig.
---

Du kan konfigurere Darkie Coder ved hjælp af en JSON-konfigurationsfil.

---

## Format

Darkie Coder understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
1. **Fjernkonfiguration** (fra `.well-known/darkie-coder`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/darkie-coder/darkie-coder.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`DARKIE_CODER_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`darkie-coder.json` i projekt) - projektspecifikke indstillinger
5. **`.darkie-coder` mapper** - agent, kommandoer, plugins
6. **Inline config** (`DARKIE_CODER_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.darkie-coder` og `~/.config/darkie-coder` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/darkie-coder`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale Darkie Coder-konfiguration i `~/.config/darkie-coder/darkie-coder.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `darkie-coder.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når Darkie Coder starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `DARKIE_CODER_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `DARKIE_CODER_CONFIG_DIR`
miljøvariabel. Dette kort vil blive søgt efter agenter, kommandoer,
modes og plugins ligesom standard `.darkie-coder` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.darkie-coder` mapper, så den **kan tilsidesætte** deres indstillinger.

---

## Skema

Konfigurationsfilen har et skema, der defineres i [**`darkie-coder.ai/config.json`**](https://darkie-coder.ai/config.json).
...
Du kan konfigurere serverindstillinger for kommandoerne `darkie-coder serve` og `darkie-coder web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din Darkie Coder-server.
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `darkie-coder.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din Darkie Coder-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger Darkie Coder at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din Darkie Coder-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/darkie-coder/agents/` eller `.darkie-coder/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil Darkie Coder falde tilbage til `"build"` med en advarsel.

Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`darkie-coder run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/darkie-coder/commands/` eller `.darkie-coder/commands/`. [Learn more here](/docs/commands).
...
Darkie Coder vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
Som standard darkie-coder **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
[Plugins](/docs/plugins) udvide Darkie Coder med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.darkie-coder/plugins/` eller `~/.config/darkie-coder/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse Darkie Coder til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
