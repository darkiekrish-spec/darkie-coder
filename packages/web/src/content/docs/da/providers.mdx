---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Darkie Coder.
---

import config from "../../../../config.mjs"
export const console = config.console

Darkie Coder bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Darkie Coder-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/darkie-coder/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Darkie Coder-konfigurationen.
...
Darkie Coder Zen er en liste over modeller leveret af Darkie Coder-teamet, som er blevet testet og verificeret til at fungere godt med Darkie Coder. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Darkie Coder Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg darkie-coder og gå til [darkie-coder.ai/auth](https://darkie-coder.ai/auth).
...
Det fungerer som alle andre udbydere i Darkie Coder og er helt valgfrit at bruge.

---

## Darkie Coder Go

Darkie Coder Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Darkie Coder-teamet, som er testet og verificeret til at fungere godt med Darkie Coder.

1. Kør kommandoen `/connect` i TUI, vælg `Darkie Coder Go`, og gå til [darkie-coder.ai/auth](https://darkie-coder.ai/zen).
...
Sådan bruger du Amazon Bedrock med Darkie Coder:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører darkie-coder:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `darkie-coder.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Darkie Coder understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere darkie-coder til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at darkie-coder skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem darkie-coder-konfigurationen.
...
2. Vælg din godkendelsesmetode:
   ...
   3. Navn: `Darkie Coder`, omfang: `api`
...
gemme tokenet i darkie-coder auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Darkie Coder bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Darkie Coder
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`darkie-coder.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [darkie-coder-gitlab-auth](https://www.npmjs.com/package/darkie-coder-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `darkie-coder.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med Darkie Coder:
...
Sådan bruger du Google Vertex AI med Darkie Coder:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører darkie-coder.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem darkie-coder, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [darkie-coder-helicone-session](https://github.com/H2Shami/darkie-coder-helicone-session) plugin for automatisk at logge hver Darkie Coder-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Darkie Coder-samtale opført som en separat session.
...
Du kan konfigurere darkie-coder til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere darkie-coder til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere darkie-coder til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Darkie Coder. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/darkie-coder) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Darkie Coder:
...
3. Kopier API-nøglen til brug i Darkie Coder.
...
6. **Vigtigt**: Før du bruger skymodeller i Darkie Coder, skal du hente modelinformationen lokalt:
...
Darkie Coder Zen er en liste over testede og verificerede modeller leveret af Darkie Coder-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Darkie Coder Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Darkie Coder Zen**.

   ```txt
   /connect
   ```

3. Indtast Darkie Coder API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem darkie-coder-konfigurationen.
...
5. Du kan også tilpasse dem gennem darkie-coder-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Darkie Coder:
...
Du kan også tilpasse modeller gennem darkie-coder-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med darkie-coder. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `darkie-coder.json`-filen i projektkataloget:
...
`limit`-felterne lader Darkie Coder forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `darkie-coder auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek darkie-coder-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i darkie-coder-konfigurationen.
