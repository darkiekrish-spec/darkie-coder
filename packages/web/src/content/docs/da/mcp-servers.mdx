...
Du kan tilføje eksterne værktøjer til Darkie Coder ved hjælp af _Model Context Protocol_ eller MCP. Darkie Coder understøtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i din [Darkie Coder Config](https://darkie-coder.ai/docs/config/) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
Organisationer kan levere standard MCP-servere via deres `.well-known/darkie-coder`-slutpunkt. Disse servere kan være deaktiveret som standard, hvilket giver brugerne mulighed for at tilmelde sig dem, de har brug for.
...
Darkie Coder håndterer automatisk OAuth-godkendelse for eksterne MCP-servere. Når en server kræver godkendelse, vil Darkie Coder:
...
Hvis serveren kræver godkendelse, vil Darkie Coder bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `darkie-coder mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil Darkie Coder opbevare tokens sikkert i `~/.local/share/darkie-coder/mcp-auth.json`.
...
Dine MCPs er tilgængelige som værktøjer i **TK\_** sammen med indbyggede værktøjer. Så du kan administrere dem gennem Darkie Coder-konfigurationen som ethvert andet værktøj.
...
Dette åbner et browservindue for at fuldføre OAuth-flowet og forbinde Darkie Coder til din Sentry-konto.
