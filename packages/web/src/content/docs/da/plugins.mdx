---
title: Plugins
description: Skriv dine egne plugins for at udvide Darkie Coder.
---

Plugins giver dig mulighed for at udvide Darkie Coder ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Darkie Coders standardadfærd.
...
- `.darkie-coder/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/darkie-coder/node_modules/`.
...
1. Global konfiguration (`~/.config/darkie-coder/darkie-coder.json`)
2. Projektkonfiguration (`darkie-coder.json`)
3. Global plugin-mappe (`~/.config/darkie-coder/plugins/`)
4. Projekt plugin bibliotek (`.darkie-coder/plugins/`)
...
Darkie Coder kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En darkie-coder SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide darkie-coder.
...
:::note
Hvis du bruger Darkie Coder desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå darkie-coder i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til darkie-coder:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som darkie-coder kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for darkie-coder sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://darkie-coder.ai/docs/sdk) for detaljer.
