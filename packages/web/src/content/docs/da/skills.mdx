...
Agentferdigheter lar Darkie Coder oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
Darkie Coder søker etter disse stedene:

- Prosjektkonfigurasjon: `.darkie-coder/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/darkie-coder/skills/<name>/SKILL.md`
...
For prosjektlokale stier går Darkie Coder opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.darkie-coder/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/darkie-coder/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.darkie-coder/skills/git-release/SKILL.md` slik:
...
Darkie Coder viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `darkie-coder.json`:
...
**For innebygde agenter** (i `darkie-coder.json`):
