...
Med Darkie Coder kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Darkie Coder vårt eget `darkie-coder`-tema.
...
Darkie Coder kommer med flere innebygde temaer.
...
- Vil at Darkie Coder skal matche terminalens utseende
...
Darkie Coder støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/darkie-coder/themes/*.json` eller `$XDG_CONFIG_HOME/darkie-coder/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.darkie-coder/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.darkie-coder/themes/*.json`
