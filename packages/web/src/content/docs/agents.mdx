...
There are two types of agents in Darkie Coder; primary agents and subagents.
...
Darkie Coder comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
Darkie Coder comes with three built-in subagents, **General**, **Explore**, and **Scout**. We'll look at this below.

---

## Built-in

Darkie Coder comes with two built-in primary agents and three built-in subagents.
...
A read-only agent for external docs and dependency research. Use this when you need to clone a dependency repository into Darkie Coder's managed cache, inspect library source, or cross-reference local code against upstream implementations without modifying your workspace.
...
Configure agents in your `darkie-coder.json` config file:
...
- Per-project: `.darkie-coder/agents/`
...
If no temperature is specified, Darkie Coder uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global Darkie Coder config and the project specific config.
...
The model ID in your Darkie Coder config uses the format `provider/model-id`. For example, if you're using [Darkie Coder Zen](/docs/zen), you would use `darkie-coder/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Run `darkie-coder models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/anomalyco/darkie-coder).
:::
