---
title: Server
description: Interact with darkie-coder server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `darkie-coder serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an darkie-coder client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `darkie-coder.local` |
...
Set `DARKIE_CODER_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `darkie-coder`, or set `DARKIE_CODER_SERVER_USERNAME` to override it. This applies to both `darkie-coder serve` and `darkie-coder web`.
...
When you run `darkie-coder` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the darkie-coder server to interact with darkie-coder programmatically.
:::

This architecture lets darkie-coder support multiple clients and allows you to interact with darkie-coder programmatically.

You can run `darkie-coder serve` to start a standalone server. If you have the
darkie-coder TUI running, `darkie-coder serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Darkie Coder [IDE](/docs/ide) plugins.
...
The darkie-coder server exposes the following APIs.
