...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. darkie-coder yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
darkie-coder içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `DARKIE_CODER_EXPERIMENTAL_LSP_TOOL=true` (veya `DARKIE_CODER_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca darkie-coder provider kullanılırken veya `DARKIE_CODER_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

darkie-coder başlatırken etkinleştirmek için:
