...
darkie-coder ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak darkie-coder kendi `darkie-coder` temasını kullanır.
...
darkie-coder birden fazla yerleşik temayla gelir.
...
- darkie-coder'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
darkie-coder, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/darkie-coder/themes/*.json` veya `$XDG_CONFIG_HOME/darkie-coder/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.darkie-coder/themes/*.json`
4. **Geçerli çalışma dizini** - `./.darkie-coder/themes/*.json`
