...
Darkie Coder Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli **aylık 10$ aboneliğidir**.

Go, Darkie Coder'daki diğer sağlayıcılar gibi çalışır. Darkie Coder Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Darkie Coder'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Darkie Coder Go, bu modellere **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Darkie Coder Go, Darkie Coder'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Darkie Coder Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Darkie Coder Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Darkie Coder Go'ya abone olabilir.
:::
...
Darkie Coder Go, [Darkie Coder](https://darkie-coder.ai) ve benzer türde istekler üreten diğer kodlama aracıları için tasarlanmıştır.
...
3. Yönlendirmeyi ve istem önbelleğe almayı optimize edebilmemiz için her konuşmada `x-darkie-coder-session` içinde değişmeyen bir oturum kimliği göndermelidir.

### Doğrulanmış İstemciler

Darkie Coder'un yanı sıra aşağıdaki istemcilerin Darkie Coder Go ile düzgün çalıştığı doğrulanmıştır.
Ancak gelecekte de çalışmaya devam edeceklerini garanti etmiyoruz.

| İstemci           | Oturum desteği                                                                                                                                                                                                                                               |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Hermes**        | [PR #101864](https://github.com/NousResearch/hermes-agent/pull/101864) içeren derlemeler, ana ve yardımcı Darkie Coder isteklerinde başlığı gönderir. Düzeltme v0.21.0'dan sonra birleştirilmiştir; dolayısıyla v0.21.0 sürümü tek başına bu düzeltmeyi içermez. |
...
| **ZCode**         | Go, ZCode'un yerel oturum başlığını tanır. [`x-darkie-coder-session` talebimiz](https://github.com/zai-org/feedback/issues/492) hâlâ açıktır ancak artık özellikle bu başlığın gönderilmesi gerekli değildir.                                                    |
| **Pi**            | Güncel derlemeler Darkie Coder için oturum bilgilerini gönderir. Eski kurulumları güncelleyin.                                                                                                                                                                   |
...
| **Kilo Code CLI** | [PR #13752](https://github.com/Kilo-Org/kilocode/pull/13752) içeren derlemeler Darkie Coder oturum başlıklarını geri getirir. Bu düzeltme CLI'ı kapsar, VS Code uzantısını kapsamaz. Bkz. [issue #13723](https://github.com/Kilo-Org/kilocode/issues/13723).     |
...
Darkie Coder yapılandırmanızdaki [model id](/docs/config/#models) formatı `darkie-coder-go/<model-id>` şeklindedir. Örneğin, Kimi K3 için yapılandırmanızda `darkie-coder-go/kimi-k3` kullanmalısınız.
...
Darkie Coder Go'yu şu amaçlarla oluşturduk:
...
4. Darkie Coder ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
