...
_Model Bağlam Protokolü_ veya MCP kullanarak darkie-coder'a harici araçlar ekleyebilirsiniz. darkie-coder hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [darkie-coder Config](https://darkie-coder.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/darkie-coder` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
darkie-coder, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde darkie-coder şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa darkie-coder, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `darkie-coder mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından darkie-coder, belirteçleri `~/.local/share/darkie-coder/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra darkie-coder'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi darkie-coder yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve darkie-coder'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
