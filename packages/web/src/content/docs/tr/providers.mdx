---
title: Sağlayıcılar
description: darkie-coder'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

darkie-coder, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı darkie-coder yapılandırmanızda yapılandırın.
...
`~/.local/share/darkie-coder/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları darkie-coder'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Darkie Coder Zen, darkie-coder ekibi tarafından sağlanan modellerin bir listesidir.
darkie-coder ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Darkie Coder Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, darkie-coder'u seçin ve [darkie-coder.ai/auth](https://darkie-coder.ai/auth)'ye gidin.
...
darkie-coder'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Darkie Coder Go

Darkie Coder Go, darkie-coder ile iyi çalıştığı test edilmiş ve doğrulanmış, darkie-coder ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Darkie Coder Go`'yu seçin ve [darkie-coder.ai/auth](https://darkie-coder.ai/zen) adresine gidin.
...
Amazon Bedrock'u darkie-coder ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   darkie-coder'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `darkie-coder.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi darkie-coder'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
darkie-coder'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   darkie-coder'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Darkie Coder yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
:::note[uyumluluk notu]
darkie-coder, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. darkie-coder'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`darkie-coder.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [darkie-coder-gitlab-auth](https://www.npmjs.com/package/darkie-coder-gitlab-auth) ana sayfasında.

##### Yapılandırma

`darkie-coder.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi darkie-coder'la kullanmak için:
...
Google Vertex AI'yi darkie-coder ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları darkie-coder çalıştırılırken ayarlayın.
...
Helicone'un darkie-coder aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her darkie-coder görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [darkie-coder-helicone-session](https://github.com/H2Shami/darkie-coder-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her darkie-coder görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
darkie-coder'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla darkie-coder'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla darkie-coder'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini darkie-coder için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/darkie-coder)'a bakın.
:::
...
Ollama Cloud'u darkie-coder ile kullanmak için:
...
3. darkie-coder'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: darkie-coder'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Darkie Coder Zen, darkie-coder ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Darkie Coder Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Darkie Coder Zen**'i arayın.

   ```txt
   /connect
   ```

3. darkie-coder API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   darkie-coder yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca darkie-coder yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Darkie Coder ile kullanmak için:
...
Modelleri darkie-coder yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı darkie-coder ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `darkie-coder.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları darkie-coder'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `darkie-coder auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için darkie-coder yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, darkie-coder yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
