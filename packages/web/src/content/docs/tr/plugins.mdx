---
title: Eklentiler
description: Darkie Coder'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Darkie Coder'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Darkie Coder'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.darkie-coder/plugins/` - Proje düzeyinde eklentiler
- `~/.config/darkie-coder/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/darkie-coder/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/darkie-coder/darkie-coder.json`)
2. Project config (`darkie-coder.json`)
3. Global eklenti dizini (`~/.config/darkie-coder/plugins/`)
4. Proje eklenti dizini (`.darkie-coder/plugins/`)
...
Darkie Coder bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Darkie Coder SDK istemcisi.
...
Darkie Coder'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Darkie Coder masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Darkie Coder'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Darkie Coder'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Darkie Coder'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Darkie Coder'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://darkie-coder.ai/docs/sdk) bakın.
