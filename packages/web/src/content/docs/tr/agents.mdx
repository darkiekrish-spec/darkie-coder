...
darkie-coder'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
darkie-coder, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
Darkie Coder, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

Darkie Coder iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
Agent'ları `darkie-coder.json` yapılandırma dosyanızda yapılandırın:
...
- Proje başına: `.darkie-coder/agents/`
...
Sıcaklık belirtilmezse darkie-coder modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global darkie-coder yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
darkie-coder yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [Darkie Coder Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `darkie-coder/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `darkie-coder models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/anomalyco/darkie-coder).
:::
