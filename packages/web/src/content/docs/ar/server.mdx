---
title: الخادم
description: تفاعل مع خادم darkie-coder عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `darkie-coder serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل darkie-coder استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `darkie-coder.local` |
...
عيّن `DARKIE_CODER_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `darkie-coder`، أو عيّن `DARKIE_CODER_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `darkie-coder serve` و `darkie-coder web`.
...
عند تشغيل `darkie-coder` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم darkie-coder للتفاعل مع darkie-coder برمجيا.
:::

تتيح هذه البنية لـ darkie-coder دعم عدة عملاء وتمكّنك من التفاعل مع darkie-coder برمجيا.

يمكنك تشغيل `darkie-coder serve` لبدء خادم مستقل. إذا كانت واجهة darkie-coder في terminal (TUI)
قيد التشغيل، فسيبدأ `darkie-coder serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Darkie Coder لـ [IDE](/docs/ide).
...
يعرض خادم darkie-coder واجهات API التالية.
