---
title: TUI
description: استخدام واجهة المستخدم TUI في Darkie Coder.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يوفّر Darkie Coder واجهة terminal تفاعلية (TUI) للعمل على مشاريعك باستخدام LLM.

يؤدي تشغيل Darkie Coder إلى بدء واجهة TUI للدليل الحالي.
...
عند استخدام واجهة Darkie Coder في terminal، يمكنك كتابة `/` متبوعة باسم أمر لتنفيذ الإجراءات بسرعة. مثلا:
...
أضف موفّرا إلى Darkie Coder. يتيح لك اختيار أحد الموفّرين المتاحين وإضافة مفاتيح API الخاصة بهم.
...
اخرج من Darkie Coder. _الأسماء المستعارة_: `/quit`, `/q`
...
هذا منفصل عن `darkie-coder.json`، الذي يضبط سلوك الخادم ووقت التشغيل.
...
- `leader_timeout` - يتحكم في مدة انتظار Darkie Coder بعد الضغط على leader key. القيمة الافتراضية `2000`.
...
- `sound_pack` - sound pack ID المراد استخدامه. القيمة الافتراضية `darkie-coder.default`.
