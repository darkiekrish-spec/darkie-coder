...
تتيح مهارات الوكيل لـ Darkie Coder اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث Darkie Coder في هذه المواقع:

- إعدادات المشروع: `.darkie-coder/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/darkie-coder/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج Darkie Coder صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.darkie-coder/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/darkie-coder/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.darkie-coder/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض Darkie Coder المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `darkie-coder.json`:
...
**للوكلاء المدمجين** (في `darkie-coder.json`):
