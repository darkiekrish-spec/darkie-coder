---
title: المزوّدون
description: استخدام أي موفّر LLM في Darkie Coder.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Darkie Coder ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Darkie Coder.
...
في `~/.local/share/darkie-coder/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Darkie Coder.
...
Darkie Coder Zen هي قائمة نماذج يوفّرها فريق Darkie Coder وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Darkie Coder. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Darkie Coder Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر darkie-coder، ثم انتقل إلى [darkie-coder.ai/auth](https://darkie-coder.ai/auth).
...
يعمل مثل أي مزوّد آخر في Darkie Coder واستخدامه اختياري بالكامل.

---

## Darkie Coder Go

Darkie Coder Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Darkie Coder، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Darkie Coder.

1. شغّل الأمر `/connect` في TUI، واختر `Darkie Coder Go`، ثم انتقل إلى [darkie-coder.ai/auth](https://darkie-coder.ai/zen).
...
لاستخدام Amazon Bedrock مع Darkie Coder:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل darkie-coder:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `darkie-coder.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Darkie Coder غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين darkie-coder لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل darkie-coder بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات darkie-coder.
...
تخزين الرمز في مساحة تخزين مصادقة darkie-coder.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Darkie Coder نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Darkie Coder
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`darkie-coder.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [darkie-coder-gitlab-auth](https://www.npmjs.com/package/darkie-coder-gitlab-auth).

##### التهيئة

خصّص عبر `darkie-coder.json`:
...
لاستخدام اشتراك GitHub Copilot مع darkie-coder:
...
لاستخدام Google Vertex AI مع Darkie Coder:
...
2. عيّن متغيرات البيئة المطلوبة:
   ...
   عيّنها أثناء تشغيل darkie-coder.
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر darkie-coder، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [darkie-coder-helicone-session](https://github.com/H2Shami/darkie-coder-helicone-session) لتسجيل كل محادثة Darkie Coder تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Darkie Coder مدرجة كجلسة مستقلة.
...
يمكنك ضبط darkie-coder لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط darkie-coder لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط darkie-coder لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Darkie Coder. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/darkie-coder) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Darkie Coder:
...
3. انسخ مفتاح API لاستخدامه في Darkie Coder.
...
6. **مهم**: قبل استخدام نماذج السحابة في Darkie Coder، يجب سحب معلومات النموذج محليا:
...
Darkie Coder Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Darkie Coder. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Darkie Coder Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Darkie Coder Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Darkie Coder.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات darkie-coder.
...
5. يمكنك أيضا تخصيصها عبر إعدادات darkie-coder. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Darkie Coder:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات darkie-coder. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع darkie-coder. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `darkie-coder.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Darkie Coder بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `darkie-coder auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات darkie-coder وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات darkie-coder.
