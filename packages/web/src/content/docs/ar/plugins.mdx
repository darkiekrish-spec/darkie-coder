---
title: الإضافات
description: اكتب إضافاتك الخاصة لتوسيع Darkie Coder.
---

تتيح لك الإضافات توسيع Darkie Coder عبر ربطها بأحداث مختلفة وتخصيص السلوك. يمكنك إنشاء إضافات لإضافة ميزات جديدة، أو التكامل مع خدمات خارجية، أو تعديل السلوك الافتراضي لـ Darkie Coder.
...
- `.darkie-coder/plugins/` - إضافات على مستوى المشروع
- `~/.config/darkie-coder/plugins/` - إضافات عامة
...
**إضافات npm** تُثبَّت تلقائيا باستخدام Bun عند بدء التشغيل. تُخزَّن الحزم واعتمادياتها مؤقتا في `~/.cache/darkie-coder/node_modules/`.
...
1. إعدادات عامة (`~/.config/darkie-coder/darkie-coder.json`)
2. إعدادات المشروع (`darkie-coder.json`)
3. دليل الإضافات العام (`~/.config/darkie-coder/plugins/`)
4. دليل إضافات المشروع (`.darkie-coder/plugins/`)
...
يشغّل Darkie Coder الأمر `bun install` عند بدء التشغيل لتثبيتها. بعد ذلك يمكن لإضافاتك وأدواتك استيرادها.
...
- `client`: عميل SDK لـ darkie-coder للتفاعل مع الذكاء الاصطناعي.
...
فيما يلي بعض أمثلة الإضافات التي يمكنك استخدامها لتوسيع darkie-coder.
...
:::note
إذا كنت تستخدم تطبيق Darkie Coder لسطح المكتب، فيمكنه إرسال إشعارات النظام تلقائيا عند جاهزية الرد أو عند حدوث خطأ في جلسة.
:::
...
امنع darkie-coder من قراءة ملفات `.env`:
...
يمكن للإضافات أيضا إضافة أدوات مخصصة إلى darkie-coder:
...
تقوم الدالة المساعدة `tool` بإنشاء أداة مخصصة يمكن لـ darkie-coder استدعاؤها. تأخذ دالة مخطط Zod وتعيد تعريف أداة يتضمن:

- `description`: ما الذي تفعله الأداة
- `args`: مخطط Zod لوسائط الأداة
- `execute`: الدالة التي تُشغَّل عند استدعاء الأداة

ستكون أدواتك المخصصة متاحة لـ darkie-coder إلى جانب الأدوات المضمنة.
...
المستويات: `debug`, `info`, `warn`, `error`. راجع [توثيق SDK](https://darkie-coder.ai/docs/sdk) للتفاصيل.
