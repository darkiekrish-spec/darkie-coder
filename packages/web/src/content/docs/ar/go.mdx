...
Darkie Coder Go هو اشتراك منخفض التكلفة بقيمة **$10/شهريًا** يمنحك وصولًا موثوقًا إلى نماذج البرمجة المفتوحة الشائعة.

يعمل Go مثل أي مزود آخر في Darkie Coder. تشترك في Darkie Coder Go وتحصل على مفتاح API الخاص بك. وهو **اختياري تمامًا**، ولا تحتاج إلى استخدامه لاستخدام Darkie Coder.
...
:::tip
اختبرنا مجموعة مختارة من النماذج والمزودين الذين يعملون جيدًا مع Darkie Coder.
:::
...
يمنحك Darkie Coder Go الوصول إلى هذه النماذج مقابل **$10/شهريًا**.

---

## كيف يعمل

يعمل Darkie Coder Go مثل أي مزود آخر في Darkie Coder.

1. تسجّل الدخول إلى **<a href={console}>Darkie Coder Zen</a>**، وتشترك في Go، ثم تنسخ مفتاح API الخاص بك.
2. تشغّل الأمر `/connect` في TUI، وتختار `Darkie Coder Go`، ثم تلصق مفتاح API الخاص بك.
3. شغّل `/models` في TUI لرؤية قائمة النماذج المتاحة عبر Go.

:::note
يمكن لعضو واحد فقط في كل workspace الاشتراك في Darkie Coder Go.
:::
...
صُمّم Darkie Coder Go للاستخدام مع [Darkie Coder](https://darkie-coder.ai) ووكلاء البرمجة الآخرين
...
3. يرسل معرّف جلسة ثابتًا في `x-darkie-coder-session` لكل محادثة حتى نتمكن من تحسين التوجيه
   والتخزين المؤقت للمطالبات.

### العملاء الذين تم التحقق منهم

بالإضافة إلى Darkie Coder، تم التحقق من أن العملاء التاليين يعملون بصورة صحيحة
مع Darkie Coder Go، مع أننا لا نضمن استمرارهم في العمل مستقبلًا.

| العميل            | دعم الجلسات                                                                                                                                                                                                                      |
| ----------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Hermes**        | ترسل البُنى التي تتضمن [PR #101864](https://github.com/NousResearch/hermes-agent/pull/101864) الترويسة في طلبات Darkie Coder الرئيسية والمساعدة. دُمج الإصلاح بعد v0.21.0؛ ولا يتضمن ذلك الإصدار وحده الإصلاح.                       |
...
| **ZCode**         | يتعرف Go على ترويسة الجلسة الأصلية الخاصة به. لا يزال [طلبنا لإضافة `x-darkie-coder-session`](https://github.com/zai-org/feedback/issues/492) مفتوحًا، لكن إرسال هذه الترويسة بعينها لم يعد ضروريًا.                                 |
| **Pi**            | ترسل الإصدارات الحالية معلومات الجلسة إلى Darkie Coder. حدّث عمليات التثبيت الأقدم.                                                                                                                                                  |
| **jcode**         | حدّث إلى **v0.81.6 أو إصدار أحدث**، إذ يتضمن [إصلاح ترويسة الجلسة](https://github.com/1jehuang/jcode/issues/1167).                                                                                                               |
| **Kilo Code CLI** | تعيد الإصدارات التي تتضمن [PR #13752](https://github.com/Kilo-Org/kilocode/pull/13752) ترويسات جلسات Darkie Coder. يشمل هذا الإصلاح CLI، وليس إضافة VS Code. راجع [issue #13723](https://github.com/Kilo-Org/kilocode/issues/13723). |
...
يستخدم [model id](/docs/config/#models) في إعدادات Darkie Coder لديك التنسيق `darkie-coder-go/<model-id>`. على سبيل المثال، بالنسبة إلى Kimi K3، ستستخدم `darkie-coder-go/kimi-k3` في إعداداتك.
...
4. **عدم فرض قيود ارتباط** من خلال السماح لك أيضًا باستخدام أي مزود آخر مع Darkie Coder.
