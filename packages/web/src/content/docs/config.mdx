---
title: Config
description: Using the Darkie Coder JSON config.
---

You can configure Darkie Coder using a JSON config file.

---

## Format

Darkie Coder supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="darkie-coder.jsonc"
{
...
1. **Remote config** (from `.well-known/darkie-coder`) - organizational defaults
2. **Global config** (`~/.config/darkie-coder/darkie-coder.json`) - user preferences
3. **Custom config** (`DARKIE_CODER_CONFIG` env var) - custom overrides
4. **Project config** (`darkie-coder.json` in project) - project-specific settings
5. **`.darkie-coder` directories** - agents, commands, plugins
6. **Inline config** (`DARKIE_CODER_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/darkie-coder/` on macOS) - admin-controlled
...
:::note
The `.darkie-coder` and `~/.config/darkie-coder` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/darkie-coder` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/darkie-coder"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global Darkie Coder config in `~/.config/darkie-coder/darkie-coder.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/darkie-coder/tui.json`.
...
Add `darkie-coder.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When Darkie Coder starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `DARKIE_CODER_CONFIG` environment variable.

```bash
export DARKIE_CODER_CONFIG=/path/to/my/custom-config.json
darkie-coder run "Hello world"
```
...
Specify a custom config directory using the `DARKIE_CODER_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.darkie-coder` directory, and should
follow the same structure.

```bash
export DARKIE_CODER_CONFIG_DIR=/path/to/my/config-directory
darkie-coder run "Hello world"
```

The custom directory is loaded after the global config and `.darkie-coder` directories, so it **can override** their settings.
...
Drop an `darkie-coder.json` or `darkie-coder.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/darkie-coder/` |
| Linux    | `/etc/darkie-coder/`                         |
| Windows  | `%ProgramData%\darkie-coder`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, Darkie Coder reads managed preferences from the `ai.darkie-coder.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

Darkie Coder checks these paths:

1. `/Library/Managed Preferences/<user>/ai.darkie-coder.managed.plist`
2. `/Library/Managed Preferences/ai.darkie-coder.managed.plist`

The plist keys map directly to `darkie-coder.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.darkie-coder.managed` PayloadType. The Darkie Coder config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.darkie-coder.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.darkie-coder.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.darkie-coder</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`darkie-coder.ai/config.json`**](https://darkie-coder.ai/config.json).

TUI config uses [**`darkie-coder.ai/tui.json`**](https://darkie-coder.ai/tui.json).
...
Use `DARKIE_CODER_TUI_CONFIG` to point to a custom TUI config file.
...
Legacy `theme`, `keybinds`, and `tui` keys in `darkie-coder.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `darkie-coder serve` and `darkie-coder web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your Darkie Coder server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `darkie-coder.local`. Useful for running multiple instances on the same network.
...
If not specified, Darkie Coder will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your Darkie Coder config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, Darkie Coder tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny Darkie Coder actions on configured resources. Currently, policies can control which providers Darkie Coder may use.
...
Darkie Coder normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, Darkie Coder omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/darkie-coder/agents/` or `.darkie-coder/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, Darkie Coder will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`darkie-coder run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/darkie-coder/commands/` or `.darkie-coder/commands/`. [Learn more here](/docs/commands).
...
Darkie Coder uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
Darkie Coder will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, darkie-coder **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend Darkie Coder with custom tools, hooks, and integrations.

Place plugin files in `.darkie-coder/plugins/` or `~/.config/darkie-coder/plugins/`. You can also load plugins from npm through the `plugin` option.

```json title="darkie-coder.json"
{
  "$schema": "https://darkie-coder.ai/config.json",
  "plugin": ["darkie-coder-helicone-session", "@my-org/custom-plugin"]
}
```
...
This is useful when you want to restrict Darkie Coder to only use specific providers rather than disabling them one by one.
