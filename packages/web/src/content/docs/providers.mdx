---
title: Providers
description: Using any LLM provider in Darkie Coder.
---

import config from "../../../config.mjs"
export const console = config.console

Darkie Coder uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Darkie Coder config.
...
in `~/.local/share/darkie-coder/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your Darkie Coder
config.
...
```json title="darkie-coder.json" {6}
{
...
Darkie Coder Zen is a list of models provided by the Darkie Coder team that have been
tested and verified to work well with Darkie Coder. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Darkie Coder Zen.
:::

1. Run the `/connect` command in the TUI, select `Darkie Coder Zen`, and head to [darkie-coder.ai/auth](https://darkie-coder.ai/zen).
...
It works like any other provider in Darkie Coder and is completely optional to use.

---

## Darkie Coder Go

Darkie Coder Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the Darkie Coder team that have been
tested and verified to work well with Darkie Coder.

1. Run the `/connect` command in the TUI, select `Darkie Coder Go`, and head to [darkie-coder.ai/auth](https://darkie-coder.ai/zen).
...
To use Amazon Bedrock with Darkie Coder:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running darkie-coder:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY darkie-coder

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile darkie-coder

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX darkie-coder
   ```
   ...
   ***
   ...
   For project-specific or persistent configuration, use `darkie-coder.json`:
...
Darkie Coder. Anthropic explicitly prohibits this.

Previous versions of Darkie Coder came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in Darkie Coder with zero setup:
...
You can configure darkie-coder to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).

```json title="darkie-coder.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://darkie-coder.ai/config.json",
  "provider": {
    "atomic-chat": {
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for darkie-coder to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX darkie-coder
   ```
...
Darkie Coder does not query Azure management APIs or discover deployments. Select a model whose catalog name matches your deployment, or configure its deployment name explicitly:
...
Assign your identity the inference role required by the deployment: **Cognitive Services OpenAI User** for Azure OpenAI models or **Cognitive Services User** for other Foundry models. Darkie Coder refreshes access tokens through the Azure CLI, including versions earlier than 2.54.0, so you only need to sign in again when the CLI session expires.
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX darkie-coder
   ```
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your darkie-coder config.
...
Darkie Coder supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; Darkie Coder uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize Darkie Coder. Sign in and approve.

   :::note
   Darkie Coder requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, Darkie Coder surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — Darkie Coder forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in Darkie Coder is experimental. Features, configuration, and
behavior may change in future releases.
:::

Darkie Coder integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
2. Choose your authentication method:
   ...
   3. Name: `Darkie Coder`, Scopes: `api`
...
to store token in darkie-coder auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
Darkie Coder uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Darkie Coder
to only use your own GitLab-hosted instance, add the following to your
`darkie-coder.json` file. It is also recommended to disable session sharing.
...
More documentation on [darkie-coder-gitlab-auth](https://www.npmjs.com/package/darkie-coder-gitlab-auth) homepage.

##### Configuration

Customize through `darkie-coder.json`:
...
When a `duo-workflow-*` model is selected, Darkie Coder will:
...
4. Route tool execution requests through Darkie Coder's permission-gated tool system
...
{
  "$schema": "https://darkie-coder.ai/config.json",
  "plugin": ["darkie-coder-gitlab-plugin"]
}
```
...
To use your GitHub Copilot subscription with darkie-coder:
...
To use GMI Cloud with Darkie Coder:
...
To use Google Vertex AI with Darkie Coder:
...
2. Set the required environment variables:
   ...
   Set them while running darkie-coder.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id darkie-coder
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through darkie-coder, you can always configure it yourself.
...
```jsonc title="~/.config/darkie-coder/darkie-coder.jsonc"
{
  "$schema": "https://darkie-coder.ai/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [darkie-coder-helicone-session](https://github.com/H2Shami/darkie-coder-helicone-session) plugin to automatically log each Darkie Coder conversation as a session in Helicone.

```bash
npm install -g darkie-coder-helicone-session
```
...
```json title="darkie-coder.json"
{
  "plugin": ["darkie-coder-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Darkie Coder conversation listed as a separate session.
...
You can configure darkie-coder to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="darkie-coder.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://darkie-coder.ai/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure darkie-coder to use local models through LM Studio.

```json title="darkie-coder.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://darkie-coder.ai/config.json",
  "provider": {
    "lmstudio": {
...
You can configure darkie-coder to use local models through Ollama.

:::tip
Ollama can automatically configure itself for Darkie Coder. See the [Ollama integration docs](https://docs.ollama.com/integrations/darkie-coder) for details.
:::

```json title="darkie-coder.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://darkie-coder.ai/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with Darkie Coder:
...
3. Copy the API key for use in Darkie Coder.
...
6. **Important**: Before using cloud models in Darkie Coder, you must pull the model information locally:
...
Darkie Coder Zen is a list of tested and verified models provided by the Darkie Coder team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Darkie Coder Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Darkie Coder Zen**.

   ```txt
   /connect
   ```

3. Enter your Darkie Coder API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your darkie-coder config.
...
5. You can also customize them through your darkie-coder config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' darkie-coder
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group darkie-coder
   ```
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Darkie Coder:
...
Don't have a Snowflake account? [Sign up for a free trial](https://signup.snowflake.com/?utm_source=darkie-coder&utm_medium=docs&utm_campaign=cortex-provider).

darkie-coder's core workflow for coding, editing files, and running commands relies on tool calling. Only the Claude and OpenAI families within Snowflake Cortex support this. The provider is limited to those families to support the core workflow.
...
5. Complete sign-in in the browser that opens. Darkie Coder captures the OAuth callback automatically and stores the token — no copy/paste needed.
...
For CI or headless environments, set a PAT or JWT before starting darkie-coder:
...
The model catalog is provided automatically. A minimal `darkie-coder.json` is all that's needed:
...
You can also customize models through your darkie-coder config. Here's an example of specifying provider routing order.
...
2. Select **SuperGrok Subscription**. Darkie Coder opens xAI's verification link with the user code pre-populated when supported.

3. Approve the consent screen. If xAI asks for a code, enter the user code displayed by Darkie Coder. Darkie Coder polls xAI's token endpoint and stores the resulting OAuth tokens once you approve.
...
Darkie Coder refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.
...
:::tip
You can use any OpenAI-compatible provider with darkie-coder. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in darkie-coder.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `darkie-coder.json` file in your project directory:

   ```json title="darkie-coder.json" ""myprovider"" {5-15}
   {
     "$schema": "https://darkie-coder.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="darkie-coder.json" {9,11,17-20}
{
  "$schema": "https://darkie-coder.ai/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow Darkie Coder to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `darkie-coder auth list` to see if the credentials
...
2. For custom providers, check the darkie-coder config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your darkie-coder config.
