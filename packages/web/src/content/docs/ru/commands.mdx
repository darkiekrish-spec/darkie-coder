...
Создайте `.darkie-coder/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию darkie-coder или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем darkie-coder [config](/docs/config):
...
- Глобальный: `~/.config/darkie-coder/commands/`
- Для каждого проекта: `.darkie-coder/commands/`
...
darkie-coder включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
