...
Darkie Coder Goは、人気のオープンなコーディングモデルに安定してアクセスできる、低価格の**月額10ドルのサブスクリプション**です。

GoはDarkie Coderの他のプロバイダーと同様に機能します。Darkie Coder GoをサブスクライブしてAPIキーを取得します。これは**完全に任意**であり、Darkie Coderを使用するために必須ではありません。
...
:::tip
Darkie Coderでうまく動作する一部のモデルとプロバイダーをテストしました。
:::
...
Darkie Coder Goを使用すると、これらのモデルに**月額10ドル**でアクセスできます。

---

## 仕組み

Darkie Coder Goは、Darkie Coderの他のプロバイダーと同様に機能します。

1. **<a href={console}>Darkie Coder Zen</a>**にサインインし、GoをサブスクライブしてAPIキーをコピーします。
2. TUIで`/connect`コマンドを実行し、`Darkie Coder Go`を選択して、APIキーを貼り付けます。
3. TUIで`/models`を実行すると、Goを通じて利用可能なモデルのリストが表示されます。

:::note
Darkie Coder Goをサブスクライブできるのは、1つのワークスペースにつき1メンバーのみです。
:::
...
Darkie Coder Goは、[Darkie Coder](https://darkie-coder.ai)および同様の種類のリクエストを生成するその他のコーディングエージェント向けに設計されています。
...
3. ルーティングとプロンプトキャッシュを最適化できるよう、会話ごとに安定したセッションIDを`x-darkie-coder-session`で送信する。

### 動作確認済みのクライアント

Darkie Coderに加えて、以下のクライアントがDarkie Coder Goで正常に動作することを確認しています。
ただし、今後も動作し続けることを保証するものではありません。

| クライアント      | セッション対応                                                                                                                                                                                                                                                                |
| ----------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Hermes**        | [PR #101864](https://github.com/NousResearch/hermes-agent/pull/101864)を含むビルドでは、Darkie Coderのメインリクエストと補助リクエストの両方でヘッダーが送信されます。この修正はv0.21.0より後にマージされたため、v0.21.0自体には含まれていません。                                |
...
| **ZCode**         | GoはZCode固有のセッションヘッダーを認識します。[`x-darkie-coder-session`の追加リクエスト](https://github.com/zai-org/feedback/issues/492)は未解決ですが、この特定のヘッダーを送信する必要はなくなりました。                                                                       |
| **Pi**            | 現在のビルドはDarkie Coderにセッション情報を送信します。古いインストールは更新してください。                                                                                                                                                                                      |
| **jcode**         | [セッションヘッダーの修正](https://github.com/1jehuang/jcode/issues/1167)を含む**v0.81.6以降**に更新してください。                                                                                                                                                            |
| **Kilo Code CLI** | [PR #13752](https://github.com/Kilo-Org/kilocode/pull/13752)を含むビルドでは、Darkie Coderのセッションヘッダーが再び送信されます。この修正の対象はCLIであり、VS Code拡張機能ではありません。[issue #13723](https://github.com/Kilo-Org/kilocode/issues/13723)を参照してください。 |
...
Darkie Coder設定の[model id](/docs/config/#models)は、`darkie-coder-go/<model-id>`という形式を使用します。たとえば、Kimi K3の場合は、設定で`darkie-coder-go/kimi-k3`を使用します。
...
Darkie Coder Goを作成した目的は以下の通りです：
...
4. Darkie Coderで他のプロバイダーも使用できるようにすることで、**ベンダーロックインをなくす**。
