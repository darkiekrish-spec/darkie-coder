---
title: サーバー
description: HTTP 経由で Darkie Coder サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`darkie-coder serve` コマンドは、Darkie Coder クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `darkie-coder.local` |
...
HTTP 基本認証でサーバーを保護するには、`DARKIE_CODER_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `darkie-coder` になるか、`DARKIE_CODER_SERVER_USERNAME` を設定してオーバーライドします。これは、`darkie-coder serve` と `darkie-coder web` の両方に当てはまります。
...
`darkie-coder` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
Darkie Coder サーバーを使用して、プログラムで Darkie Coder と対話します。
:::

`darkie-coder serve` を実行してスタンドアロンサーバーを起動できます。すでに Darkie Coder TUI を実行している場合でも、`darkie-coder serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、Darkie Coder [IDE](/docs/ide) プラグインによって使用されます。
...
Darkie Coder サーバーは次の API を公開します。
