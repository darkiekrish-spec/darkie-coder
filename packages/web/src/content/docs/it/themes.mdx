...
Con Darkie Coder puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, Darkie Coder usa il tema `darkie-coder`.
...
Darkie Coder include diversi temi integrati.
...
- Vuole che Darkie Coder corrisponda all'aspetto del terminale
...
Darkie Coder supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/darkie-coder/themes/*.json` o `$XDG_CONFIG_HOME/darkie-coder/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.darkie-coder/themes/*.json`
4. **Directory di lavoro corrente** - in `./.darkie-coder/themes/*.json`
