---
title: Provider
description: Usa qualsiasi provider LLM in Darkie Coder.
---

import config from "../../../../config.mjs"
export const console = config.console

Darkie Coder usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di Darkie Coder.
...
configurazione di Darkie Coder.
...
Darkie Coder Zen e una lista di modelli forniti dal team Darkie Coder, testati e
verificati per funzionare bene con Darkie Coder. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da Darkie Coder Zen.
:::

1. Esegui `/connect` nella TUI, seleziona darkie-coder e vai su [darkie-coder.ai/auth](https://darkie-coder.ai/auth).
...
Funziona come qualsiasi altro provider in Darkie Coder ed e totalmente opzionale.
...
Per usare Amazon Bedrock con Darkie Coder:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui darkie-coder:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `darkie-coder.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in Darkie Coder non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
Puoi configurare darkie-coder per utilizzare modelli locali tramite [Atomic Chat](https://atomic.chat), un'applicazione desktop che esegue LLM locali dietro un server API compatibile OpenAI (endpoint predefinito `http://127.0.0.1:1337/v1`).
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché darkie-coder funzioni correttamente.
   :::
...
5. Esegui il comando `/models` per selezionare un modello.
   ...
   Puoi anche aggiungere modelli tramite la tua configurazione di darkie-coder.
...
2. Scegli il tuo metodo di autenticazione:
   ...
   3. Nome: `Darkie Coder`, Scopes: `api`
...
salvare il token nello storage di auth di darkie-coder.
:::

##### GitLab Self-Hosted

:::note[nota di compliance]
Darkie Coder usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare Darkie Coder
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `darkie-coder.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Maggior documentazione sulla homepage di [darkie-coder-gitlab-auth](https://www.npmjs.com/package/darkie-coder-gitlab-auth).

##### Configurazione

Personalizza tramite `darkie-coder.json`:
...
Per usare il tuo abbonamento GitHub Copilot con darkie-coder:
...
Per usare Google Vertex AI con Darkie Coder:
...
2. Imposta le variabili d'ambiente richieste:
   ...
   Impostale mentre esegui darkie-coder.
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da darkie-coder, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [darkie-coder-helicone-session](https://github.com/H2Shami/darkie-coder-helicone-session) per loggare automaticamente ogni conversazione di Darkie Coder come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di Darkie Coder elencata come sessione separata.
...
Puoi configurare darkie-coder per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare darkie-coder per usare modelli locali tramite LM Studio.
...
Puoi configurare darkie-coder per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per Darkie Coder. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/darkie-coder) per dettagli.
:::
...
Per usare Ollama Cloud con Darkie Coder:
...
3. Copia la chiave API da usare in Darkie Coder.
...
6. **Importante**: Prima di usare modelli cloud in Darkie Coder, devi scaricare le informazioni del modello localmente:
...
Darkie Coder Zen è una lista di modelli testati e verificati forniti dal team Darkie Coder. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>Darkie Coder Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **Darkie Coder Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di Darkie Coder.
...
4. Molti modelli OpenRouter sono precaricati di default, esegui il comando `/models` per selezionare quello che vuoi.
   ...
   Puoi anche aggiungere modelli addizionali tramite la tua configurazione di darkie-coder.
...
5. Puoi anche personalizzarli tramite la configurazione di darkie-coder. Ecco un esempio di specifica di un provider
...
Per usare le [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) con Darkie Coder:
...
Puoi anche personalizzare i modelli tramite la tua configurazione di darkie-coder. Ecco un esempio di come specificare l'ordine di routing dei provider.
...
:::tip
Puoi usare qualsiasi provider OpenAI-compatible con darkie-coder. La maggior parte dei provider AI moderni offre API OpenAI-compatible.
:::
...
4. Crea o aggiorna il tuo file `darkie-coder.json` nella directory del progetto:
...
I campi `limit` permettono a Darkie Coder di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `darkie-coder auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di darkie-coder e:
   - Assicurati che l'ID provider usato nel comando `/connect` corrisponda all'ID nella tua config darkie-coder.
