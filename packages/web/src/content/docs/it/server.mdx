---
title: Server
description: Interagisci con il server darkie-coder via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `darkie-coder serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client darkie-coder.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `darkie-coder.local` |
...
Imposta `DARKIE_CODER_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `darkie-coder`, oppure imposta `DARKIE_CODER_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `darkie-coder serve` sia per `darkie-coder web`.
...
Quando esegui `darkie-coder` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server darkie-coder per interagire con darkie-coder in modo programmatico.
:::

Questa architettura permette a darkie-coder di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `darkie-coder serve` per avviare un server standalone. Se la TUI di darkie-coder e gia in esecuzione, `darkie-coder serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Darkie Coder.
...
Il server darkie-coder espone le seguenti API.
