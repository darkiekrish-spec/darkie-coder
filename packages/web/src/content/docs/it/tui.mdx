---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di Darkie Coder.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Darkie Coder fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Darkie Coder avvia la TUI per la directory corrente.
...
Quando usi la TUI di Darkie Coder, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Darkie Coder. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da Darkie Coder. _Alias_: `/quit`, `/q`
...
Questo è separato da `darkie-coder.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo Darkie Coder attende dopo il leader key. Default: `2000`.
...
- `sound_pack` - ID del sound pack da usare. Default: `darkie-coder.default`.
