---
title: Server LSP
description: Darkie Coder si integra con i tuoi server LSP.
---

Darkie Coder può integrarsi con server Language Server Protocol (LSP) per usare la diagnostica come feedback per l'agente.

---

## Integrati

Darkie Coder include diversi server LSP integrati per linguaggi popolari:
...
:::note
Puoi disabilitare il download automatico dei server LSP impostando la variabile d'ambiente `DARKIE_CODER_DISABLE_LSP_DOWNLOAD` a `true`.
:::
...
Quando LSP è abilitato e darkie-coder apre un file:
...
Puoi abilitare e personalizzare i server LSP tramite la sezione `lsp` nella configurazione di darkie-coder.
