...
Puoi aggiungere strumenti esterni a Darkie Coder usando il _Model Context Protocol_, o MCP. Darkie Coder supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Darkie Coder Config](https://darkie-coder.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/darkie-coder`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Darkie Coder gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Darkie Coder:
...
Se il server richiede autenticazione, Darkie Coder ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `darkie-coder mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Darkie Coder memorizzera' i token in modo sicuro in `~/.local/share/darkie-coder/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Darkie Coder insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Darkie Coder come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Darkie Coder al tuo account Sentry.
