...
Le skill degli agenti permettono a Darkie Coder di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Darkie Coder cerca in queste posizioni:

- Config di progetto: `.darkie-coder/skills/<name>/SKILL.md`
- Config globale: `~/.config/darkie-coder/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Darkie Coder risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.darkie-coder/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/darkie-coder/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.darkie-coder/skills/git-release/SKILL.md` cosi':
...
Darkie Coder elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `darkie-coder.json`:
...
**Per agenti integrati** (in `darkie-coder.json`):
