---
title: Plugin
description: Scrivi plugin per estendere Darkie Coder.
---

I plugin ti permettono di estendere Darkie Coder agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Darkie Coder.
...
- `.darkie-coder/plugins/` - plugin a livello progetto
- `~/.config/darkie-coder/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/darkie-coder/node_modules/`.
...
1. Config globale (`~/.config/darkie-coder/darkie-coder.json`)
2. Config di progetto (`darkie-coder.json`)
3. Directory plugin globale (`~/.config/darkie-coder/plugins/`)
4. Directory plugin di progetto (`.darkie-coder/plugins/`)
...
Darkie Coder esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di darkie-coder per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere darkie-coder.
...
:::note
Se usi l'app desktop di Darkie Coder, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a darkie-coder di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a darkie-coder:
...
L'helper `tool` crea uno strumento personalizzato che darkie-coder può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in darkie-coder insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://darkie-coder.ai/docs/sdk) per i dettagli.
