---
title: Custom Tools
description: Create tools the LLM can call in darkie-coder.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside darkie-coder's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.darkie-coder/tools/` directory of your project.
- Or globally, by placing them in `~/.config/darkie-coder/tools/`.
