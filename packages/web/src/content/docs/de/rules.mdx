---
title: Regeln
description: Lege eigene Anweisungen fuer darkie-coder fest.
---

Du kannst darkie-coder ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in darkie-coder den Befehl `/init` aus.
...
So kann darkie-coder sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht darkie-coder sie zu erweitern.
...
darkie-coder kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/darkie-coder/AGENTS.md` speichern.
Diese gelten in allen darkie-coder-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Darkie Coder dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/darkie-coder/AGENTS.md` existiert)
...
Beim Start sucht darkie-coder in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/darkie-coder/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `darkie-coder.json` oder in `~/.config/darkie-coder/darkie-coder.json` angeben.
...
darkie-coder parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit darkie-coder.json

Empfohlen ist das `instructions`-Feld in der `darkie-coder.json`:
...
Du kannst darkie-coder explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `darkie-coder.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
