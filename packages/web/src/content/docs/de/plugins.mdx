---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Darkie Coder zu erweitern.
---

Mit Plugins können Sie Darkie Coder erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Darkie Coder zu ändern.
...
- `.darkie-coder/plugins/` – Plugins auf Projektebene
- `~/.config/darkie-coder/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/darkie-coder/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/darkie-coder/darkie-coder.json`)
2. Projektkonfiguration (`darkie-coder.json`)
3. Globales Plugin-Verzeichnis (`~/.config/darkie-coder/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.darkie-coder/plugins/`)
...
Darkie Coder führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Darkie Coder-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Darkie Coder erweitern können.
...
:::note
Wenn Sie die Darkie Coder-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Darkie Coder `.env`-Dateien liest:
...
Plugins können Darkie Coder auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Darkie Coder aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Darkie Coder zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://darkie-coder.ai/docs/sdk).
