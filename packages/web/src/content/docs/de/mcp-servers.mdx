...
Sie können externe Tools zu Darkie Coder hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. Darkie Coder unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [Darkie Coder Config](https://darkie-coder.ai/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/darkie-coder`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
Darkie Coder übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird Darkie Coder Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von Darkie Coder beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `darkie-coder mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert Darkie Coder die Token sicher in `~/.local/share/darkie-coder/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in Darkie Coder verfügbar. Sie können sie also wie jedes andere Tool über die Darkie Coder-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und Darkie Coder mit Ihrem Sentry-Konto zu verbinden.
