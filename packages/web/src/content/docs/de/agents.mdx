...
Es gibt zwei Arten von Agenten in Darkie Coder; Primäragenten und Subagenten.
...
Darkie Coder verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `darkie-coder.json`-Konfigurationsdatei:
...
- Pro Projekt: `.darkie-coder/agents/`
...
Wenn keine Temperatur angegeben ist, verwendet Darkie Coder modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale Darkie Coder-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer Darkie Coder-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [Darkie Coder Zen](/docs/zen) verwenden, würden Sie `darkie-coder/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `darkie-coder models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
...
:::tip
Haben Sie einen Agenten, den Sie gerne teilen möchten? [Reichen Sie einen PR ein](https://github.com/anomalyco/darkie-coder).
:::
