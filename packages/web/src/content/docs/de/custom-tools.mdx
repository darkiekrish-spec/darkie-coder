...
description: Erstellen Sie Tools, die der LLM in Darkie Coder aufrufen kann.
---

Benutzerdefinierte Tools sind von Ihnen erstellte Funktionen, die der LLM während Gesprächen aufrufen kann. Sie arbeiten neben [built-in tools](/docs/tools) von Darkie Coder wie `read`, `write` und `bash`.
...
- Lokal, indem Sie sie im Verzeichnis `.darkie-coder/tools/` Ihres Projekts platzieren.
- Oder global, indem Sie sie in `~/.config/darkie-coder/tools/` platzieren.
