---
title: Server
description: Nutze den darkie-coder-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `darkie-coder serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein darkie-coder-Client nutzen kann.
...
Setze `DARKIE_CODER_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `darkie-coder`, kann aber mit `DARKIE_CODER_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `darkie-coder serve` und `darkie-coder web`.
...
Wenn du `darkie-coder` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den darkie-coder-Server, um darkie-coder programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `darkie-coder serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `darkie-coder serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Darkie Coder-[IDE](/docs/ide)-Plugins.
...
Der darkie-coder-Server stellt folgende APIs bereit.
