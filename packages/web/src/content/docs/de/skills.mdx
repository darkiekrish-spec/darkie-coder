...
Agent Skills erlauben Darkie Coder, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Darkie Coder sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Darkie Coder vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.darkie-coder/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/darkie-coder/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.darkie-coder/skills/git-release/SKILL.md` zum Beispiel so:
...
Darkie Coder listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `darkie-coder.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `darkie-coder.json`):
