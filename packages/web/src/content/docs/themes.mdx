...
With Darkie Coder you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Darkie Coder uses our own `darkie-coder` theme.
...
Darkie Coder comes with several built-in themes.
...
- Want Darkie Coder to match their terminal's appearance
...
Darkie Coder supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/darkie-coder/themes/*.json` or `$XDG_CONFIG_HOME/darkie-coder/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.darkie-coder/themes/*.json`
4. **Current working directory** - Defined in `./.darkie-coder/themes/*.json`
