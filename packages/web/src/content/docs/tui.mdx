---
title: TUI
description: Using the Darkie Coder terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Darkie Coder provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Darkie Coder starts the TUI for the current directory.
...
When using the Darkie Coder TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Darkie Coder. Allows you to select from available providers and add their API keys.
...
Exit Darkie Coder. _Aliases_: `/quit`, `/q`
...
This is separate from `darkie-coder.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long Darkie Coder waits after the leader key. Defaults to `2000`.
...
Use `DARKIE_CODER_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `darkie-coder.default`.
