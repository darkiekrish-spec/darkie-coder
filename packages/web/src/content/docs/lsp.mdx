---
title: LSP Servers
description: Darkie Coder integrates with your LSP servers.
---

Darkie Coder can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent.

---

## Built-in

Darkie Coder comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `DARKIE_CODER_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and darkie-coder opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your darkie-coder config.
...
```json title="darkie-coder.json" {5-8}
{
  "$schema": "https://darkie-coder.ai/config.json",
  "lsp": {
    "rust": {
...
```json title="darkie-coder.json" {5-13}
{
  "$schema": "https://darkie-coder.ai/config.json",
  "lsp": {
    "custom-lsp": {
