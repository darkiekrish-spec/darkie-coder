...
As habilidades do agente permitem que o darkie-coder descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O darkie-coder pesquisa nesses locais:

- Configuração do projeto: `.darkie-coder/skills/<name>/SKILL.md`
- Configuração global: `~/.config/darkie-coder/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o darkie-coder sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.darkie-coder/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/darkie-coder/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.darkie-coder/skills/git-release/SKILL.md` assim:
...
O darkie-coder lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `darkie-coder.json`:
...
**Para agentes embutidos** (em `darkie-coder.json`):
