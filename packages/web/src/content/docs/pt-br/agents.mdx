...
Existem dois tipos de agentes no darkie-coder; agentes primários e subagentes.
...
darkie-coder vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
Darkie Coder vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

Darkie Coder vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do Darkie Coder, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `darkie-coder.json`:
...
- Global: `~/.config/darkie-coder/agents/`
- Por projeto: `.darkie-coder/agents/`

```markdown title="~/.config/darkie-coder/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o darkie-coder usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do darkie-coder quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do darkie-coder usa o formato `provider/model-id`. Por exemplo, se você estiver usando [Darkie Coder Zen](/docs/zen), você usaria `darkie-coder/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="darkie-coder.json" {3-6,9-12}
{
  "$schema": "https://darkie-coder.ai/config.json",
  "tools": {
    "write": true,
...
```json title="darkie-coder.json" {3-5,8-10}
{
  "$schema": "https://darkie-coder.ai/config.json",
  "permission": {
    "edit": "deny"
...
```json title="darkie-coder.json" {8}
{
  "$schema": "https://darkie-coder.ai/config.json",
  "agent": {
    "build": {
...
```json title="darkie-coder.json" {6,7}
{
  "agent": {
    "deep-thinker": {
...
:::tip
Execute `darkie-coder models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/anomalyco/darkie-coder).
:::
...
```markdown title="~/.config/darkie-coder/agents/docs-writer.md"
---
...
```markdown title="~/.config/darkie-coder/agents/security-auditor.md"
---
