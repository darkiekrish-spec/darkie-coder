...
Com o darkie-coder, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o darkie-coder usa nosso próprio tema `darkie-coder`.
...
O darkie-coder vem com vários temas embutidos.
...
- Querem que o darkie-coder corresponda à aparência do seu terminal
...
O darkie-coder suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/darkie-coder/themes/*.json` ou `$XDG_CONFIG_HOME/darkie-coder/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.darkie-coder/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.darkie-coder/themes/*.json`
