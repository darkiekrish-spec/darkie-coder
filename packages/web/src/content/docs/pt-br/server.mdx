---
title: Servidor
description: Interaja com o servidor darkie-coder via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `darkie-coder serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente darkie-coder pode usar.
...
```bash
darkie-coder serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `darkie-coder.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
darkie-coder serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `DARKIE_CODER_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `darkie-coder`, ou defina `DARKIE_CODER_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `darkie-coder serve` quanto ao `darkie-coder web`.

```bash
DARKIE_CODER_SERVER_PASSWORD=your-password darkie-coder serve
```
...
Quando você executa `darkie-coder`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor darkie-coder para interagir com o darkie-coder programaticamente.
:::

Essa arquitetura permite que o darkie-coder suporte múltiplos clientes e permite que você interaja com o darkie-coder programaticamente.

Você pode executar `darkie-coder serve` para iniciar um servidor autônomo. Se você tiver o TUI do darkie-coder em execução, `darkie-coder serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do darkie-coder [IDE](/docs/ide).
...
O servidor darkie-coder expõe as seguintes APIs.
