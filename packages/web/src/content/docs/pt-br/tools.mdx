...
As ferramentas permitem que o LLM execute ações em sua base de código. O darkie-coder vem com um conjunto de ferramentas integradas, mas você pode estendê-lo com [ferramentas personalizadas](/docs/custom-tools) ou [servidores MCP](/docs/mcp-servers).
...
Aqui estão todas as ferramentas integradas disponíveis no darkie-coder.
...
:::note
Esta ferramenta está disponível apenas quando `DARKIE_CODER_EXPERIMENTAL_LSP_TOOL=true` (ou `DARKIE_CODER_EXPERIMENTAL=true`).
:::
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor darkie-coder ou quando a variável de ambiente `DARKIE_CODER_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).

Para habilitar ao iniciar o darkie-coder:
