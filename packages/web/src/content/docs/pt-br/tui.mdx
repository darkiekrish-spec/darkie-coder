---
title: TUI
description: Usando a interface de usuário de terminal darkie-coder.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

O darkie-coder fornece uma interface de terminal interativa ou TUI para trabalhar em seus projetos com um LLM.

Executar darkie-coder inicia o TUI para o diretório atual.
...
Ao usar o TUI do darkie-coder, você pode digitar `/` seguido pelo nome de um comando para executar ações rapidamente. Por exemplo:
...
Adicione um provedor ao darkie-coder. Permite que você selecione entre os provedores disponíveis e adicione suas chaves de API.
...
Saia do darkie-coder. _Aliases_: `/quit`, `/q`
...
Isso é separado do `darkie-coder.json`, que configura o comportamento do servidor/runtime.
...
- `leader_timeout` - Controla por quanto tempo o Darkie Coder espera depois da leader key. O padrão é `2000`.
...
Use `DARKIE_CODER_TUI_CONFIG` para carregar um caminho de configuração TUI personalizado.

### Attention

Attention permite que o TUI avise você quando o Darkie Coder espera uma resposta, precisa da aprovação de uma permissão, encontra um erro de sessão ou conclui uma sessão. Ative com `attention.enabled`; eventos integrados reproduzem sons quando acontecem. As notificações de desktop só são enviadas quando a janela do terminal não está em foco e não são usadas para eventos de subagent.
...
- `sound_pack` - ID do sound pack a ser usado. O padrão é `darkie-coder.default`.
