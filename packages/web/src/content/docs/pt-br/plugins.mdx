---
title: Plugins
description: Escreva seus próprios plugins para estender o darkie-coder.
---

Plugins permitem que você estenda o darkie-coder conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do darkie-coder.
...
- `.darkie-coder/plugins/` - Plugins em nível de projeto
- `~/.config/darkie-coder/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/darkie-coder/node_modules/`.
...
1. Configuração global (`~/.config/darkie-coder/darkie-coder.json`)
2. Configuração do projeto (`darkie-coder.json`)
3. Diretório de plugins global (`~/.config/darkie-coder/plugins/`)
4. Diretório de plugins do projeto (`.darkie-coder/plugins/`)
...
O darkie-coder executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do darkie-coder para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o darkie-coder.
...
:::note
Se você estiver usando o aplicativo desktop darkie-coder, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o darkie-coder de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao darkie-coder:
...
O helper `tool` cria uma ferramenta personalizada que o darkie-coder pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o darkie-coder junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://darkie-coder.ai/docs/sdk) para detalhes.
