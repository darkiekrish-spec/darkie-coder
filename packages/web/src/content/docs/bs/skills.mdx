...
Agent skills omogucavaju Darkie Coder da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Darkie Coder pretrazuje ove lokacije:

- Konfiguracija projekta: `.darkie-coder/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/darkie-coder/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Darkie Coder ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.darkie-coder/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/darkie-coder/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.darkie-coder/skills/git-release/SKILL.md` ovako:
...
Darkie Coder navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `darkie-coder.json`:
...
**Za ugradene agente** (u `darkie-coder.json`):
