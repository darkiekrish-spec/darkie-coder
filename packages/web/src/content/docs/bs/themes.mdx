...
U Darkie Coder mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Darkie Coder koristi nasu `darkie-coder` temu.
...
Darkie Coder dolazi sa vise ugradenih tema.
...
- Zele da Darkie Coder odgovara izgledu njihovog terminala
...
Darkie Coder podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/darkie-coder/themes/*.json` ili `$XDG_CONFIG_HOME/darkie-coder/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.darkie-coder/themes/*.json`
4. **Trenutni radni direktorij** - `./.darkie-coder/themes/*.json`
