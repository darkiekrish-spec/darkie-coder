...
Možete dodati vanjske alate u Darkie Coder koristeći _Model Context Protocol_, ili MCP. Darkie Coder podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Darkie Coder Config](https://darkie-coder.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/darkie-coder`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Darkie Coder automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Darkie Coder će:
...
Ako server zahtijeva autentifikaciju, Darkie Coder će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `darkie-coder mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Darkie Coder sigurno čuva tokene u `~/.local/share/darkie-coder/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Darkie Coder, zajedno s ugrađenim alatima. Možete njima upravljati kroz Darkie Coder konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Darkie Coder sa vašim Sentry nalogom.
