---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Darkie Coder.
---

import config from "../../../../config.mjs"
export const console = config.console

Darkie Coder koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Darkie Coder konfiguraciji.
...
u `~/.local/share/darkie-coder/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Darkie Coder
config.
...
Darkie Coder Zen je lista modela koje je obezbedio Darkie Coder tim koji su bili
testirano i potvrđeno da dobro radi sa Darkie Coder. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Darkie Coder Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite darkie-coder i idite na [darkie-coder.ai/auth](https://darkie-coder.ai/auth).
...
Radi kao i svaki drugi provajder u Darkie Coder i potpuno je opcionalan za korištenje.

---

## Darkie Coder Go

Darkie Coder Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Darkie Coder tim i koji su testirani i verificirani da dobro rade s Darkie Coder-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Darkie Coder Go` i idite na [darkie-coder.ai/auth](https://darkie-coder.ai/zen).
...
Da biste koristili Amazon Bedrock s Darkie Coder:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete darkie-coder:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `darkie-coder.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Darkie Coder.
:::
...
Možete konfigurirati darkie-coder za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi darkie-coder ispravno radio.
   :::
...
Također možete dodati modele kroz svoju darkie-coder konfiguraciju.
...
3.  Naziv: `Darkie Coder`, opseg: `api`
...
da pohrani token u darkie-coder auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Darkie Coder koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Darkie Coder
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`darkie-coder.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [darkie-coder-gitlab-auth](https://www.npmjs.com/package/darkie-coder-gitlab-auth).

##### Konfiguracija

Prilagodite putem `darkie-coder.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s darkie-coder:
...
Za korištenje Google Vertex AI s Darkie Coder:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće darkie-coder.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem darkie-codera, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [darkie-coder-helicone-session](https://github.com/H2Shami/darkie-coder-helicone-session) da automatski prijavite svaki Darkie Coder razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Darkie Coder razgovor naveden kao zasebna sesija.
...
Možete konfigurirati darkie-coder za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati darkie-coder za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati darkie-coder za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Darkie Coder. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/darkie-coder) za detalje.
:::
...
Da biste koristili Ollama Cloud s Darkie Coder:
...
3. Kopirajte API ključ za korištenje u Darkie Coder.
...
6. **Važno**: Prije upotrebe modela oblaka u Darkie Coder, morate lokalno povući informacije o modelu:
...
Darkie Coder Zen je lista testiranih i verifikovanih modela koju je obezbedio Darkie Coder tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Darkie Coder Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Darkie Coder Zen**.

```txt
   /connect
```

3. Unesite svoj Darkie Coder API ključ.
...
Također možete dodati dodatne modele putem vaše darkie-coder konfiguracije.
...
5. Također ih možete prilagoditi putem vaše darkie-coder konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Darkie Coderom:
...
Također možete prilagoditi modele kroz svoju darkie-coder konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s darkie-coder-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `darkie-coder.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Darkie Coder da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `darkie-coder auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Darkie Coder konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj darkie-coder konfiguraciji.
