---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Darkie Coder.
---

Dodaci vam omogućavaju da proširite Darkie Coder spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Darkie Coder.
...
- `.darkie-coder/plugins/` - Dodaci na nivou projekta
- `~/.config/darkie-coder/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/darkie-coder/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/darkie-coder/darkie-coder.json`)
2. Konfiguracija projekta (`darkie-coder.json`)
3. Globalni direktorij dodataka (`~/.config/darkie-coder/plugins/`)
4. Direktorij dodataka projekta (`.darkie-coder/plugins/`)
...
Darkie Coder pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Darkie Coder SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Darkie Coder.
...
Spriječite darkie-coder da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u darkie-coder:
...
Pomoćnik `tool` kreira prilagođeni alat koji darkie-coder može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za darkie-coder zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://darkie-coder.ai/docs/sdk) za detalje.
