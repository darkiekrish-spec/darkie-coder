---
title: Server
description: Komunicirajte s darkie-coder serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `darkie-coder serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji darkie-coder klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `darkie-coder.local` |
...
Postavite `DARKIE_CODER_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `darkie-coder`, ili postavite `DARKIE_CODER_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `darkie-coder serve` i za `darkie-coder web`.
...
Kada pokrenete `darkie-coder`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite darkie-coder server za programsku interakciju sa darkie-coder-om.
:::

Ova arhitektura omogucava darkie-coder podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `darkie-coder serve` da startate standalone server. Ako je darkie-coder TUI vec pokrenut, `darkie-coder serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Darkie Coder [IDE](/docs/ide) pluginovi.
...
darkie-coder server izlaže sljedece API-je.
