---
title: Konfiguracija
description: Korištenje Darkie Coder JSON konfiguracije.
---

Možete konfigurirati Darkie Coder koristeći JSON konfiguracijski fajl.

---

## Format

Darkie Coder podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/darkie-coder`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/darkie-coder/darkie-coder.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`DARKIE_CODER_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`darkie-coder.json` u projektu) - postavke specifične za projekat
5. **`.darkie-coder` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`DARKIE_CODER_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.darkie-coder` i `~/.config/darkie-coder` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/darkie-coder` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Darkie Coder konfiguraciju u `~/.config/darkie-coder/darkie-coder.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/darkie-coder/tui.json`.
...
Dodajte `darkie-coder.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Darkie Coder pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `DARKIE_CODER_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `DARKIE_CODER_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.darkie-coder` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.darkie-coder`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`darkie-coder.ai/config.json`**](https://darkie-coder.ai/config.json).

TUI konfiguracija koristi [**`darkie-coder.ai/tui.json`**](https://darkie-coder.ai/tui.json).
...
Koristite `DARKIE_CODER_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `darkie-coder.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `darkie-coder serve` i `darkie-coder web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Darkie Coder server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `darkie-coder.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Darkie Coder konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Darkie Coder pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/darkie-coder/agents/` ili `.darkie-coder/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Darkie Coder će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`darkie-coder run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/darkie-coder/commands/` ili `.darkie-coder/commands/`. [Saznajte više ovdje](/docs/commands).
...
Darkie Coder će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Darkie Coder **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Darkie Coder sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.darkie-coder/plugins/` ili `~/.config/darkie-coder/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Darkie Coder da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
