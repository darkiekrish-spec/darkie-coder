...
Postoje dvije vrste agenata u Darkie Coder: primarni agenti i podagenti.
...
Darkie Coder dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
Darkie Coder dolazi sa tri ugrađena subagenta, **General**, **Explore** i **Scout**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

Darkie Coder dolazi sa dva ugrađena primarna agenta i tri ugrađena subagenta.
...
Agent samo za čitanje za istraživanje eksterne dokumentacije i zavisnosti. Koristite ga kada trebate klonirati repozitorij zavisnosti u Darkie Coder-ov upravljani cache, pregledati izvorni kod biblioteke ili uporediti lokalni kod sa upstream implementacijama bez mijenjanja vašeg radnog prostora.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `darkie-coder.json`:
...
- Globalno: `~/.config/darkie-coder/agents/`
- Po projektu: `.darkie-coder/agents/`
...
Ako temperatura nije navedena, Darkie Coder koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu Darkie Coder konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj Darkie Coder konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [Darkie Coder Zen](/docs/zen), koristili biste `darkie-coder/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `darkie-coder models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/anomalyco/darkie-coder).
:::
