...
Tools allow the LLM to perform actions in your codebase. Darkie Coder comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in Darkie Coder.
...
:::note
This tool is only available when `DARKIE_CODER_EXPERIMENTAL_LSP_TOOL=true` (or `DARKIE_CODER_EXPERIMENTAL=true`).
:::
...
:::note
This tool is only available when using the Darkie Coder or Darkie Coder Go provider, or when either the `DARKIE_CODER_ENABLE_EXA` or `DARKIE_CODER_ENABLE_PARALLEL` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching Darkie Coder:

```bash
DARKIE_CODER_ENABLE_EXA=1 darkie-coder
# or
DARKIE_CODER_ENABLE_PARALLEL=1 darkie-coder
```
