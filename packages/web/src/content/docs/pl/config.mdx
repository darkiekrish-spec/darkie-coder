---
title: Konfiguracja
description: Ustawienia Darkie Coder w pliku JSON.
---

Możesz dostosować Darkie Coder za pomocą pliku konfiguracyjnego JSON.

---

## Format

Darkie Coder obsługuje formaty **JSON** i **JSONC** (JSON z komentarzami).
...
Na przykład, jeśli konfiguracja globalna ustawi `theme: "darkie-coder"` i `autoupdate: true`, a konfiguracja projektu ustawi `model: "anthropic/claude-sonnet-4-5"`, ostateczna konfiguracja będzie zawierać wszystkie trzy ustawienia.
...
1. **Konfiguracja zdalna** (z `.well-known/darkie-coder`) – ustawienia wymuszane przez organizację
2. **Konfiguracja globalna** (`~/.config/darkie-coder/darkie-coder.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `DARKIE_CODER_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`darkie-coder.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.darkie-coder`** - agenci, polecenia, umiejętności
6. **Konfiguracja wbudowana** (zmienna `DARKIE_CODER_CONFIG_CONTENT`) — nadpisanie środowiska uruchomieniowego
...
:::note
W katalogach `.darkie-coder` i `~/.config/darkie-coder` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Organizacje mogą publikować konfigurację pod adresem `.well-known/darkie-coder`. Jest ona pobierana automatycznie po uwierzytelnieniu u dostawcy, który ją obsługuje.
...
Umieść swoją globalną konfigurację Darkie Coder w `~/.config/darkie-coder/darkie-coder.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/darkie-coder/tui.json`.
...
Dodaj `darkie-coder.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Kiedy Darkie Coder się uruchamia, szuka pliku konfiguracyjnego w katalogu głównym repozytorium Git.
...
Możesz załadować niestandardowy plik konfiguracyjny, ustawiając zmienną środowiskową `DARKIE_CODER_CONFIG`.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `DARKIE_CODER_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.darkie-coder` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.darkie-coder`, więc **może być łączony** z ich zawartością.

---

## Schemat

Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`darkie-coder.ai/config.json`**](https://darkie-coder.ai/config.json).
...
Użyj `DARKIE_CODER_TUI_CONFIG`, aby wskazać niestandardowy plik konfiguracyjny TUI.

Przestarzałe klucze `theme`, `keybinds` i `tui` w `darkie-coder.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
Możesz skonfigurować serwer dla `darkie-coder serve` i `darkie-coder web` za pomocą opcji `server`.
...
- `mdns` - Włącz wykrywanie usług mDNS. Dzięki temu inne urządzenia w sieci mogą łatwo znaleźć Twój serwer Darkie Coder.
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `darkie-coder.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Opcja `small_model` konfiguruje oddzielny model do lżejszych zadań, takich jak generowanie tytułów. Domyślnie Darkie Coder wybiera tańszy model od tego samego dostawcy, jeśli jest dostępny, w przeciwnym razie powraca do modelu głównego.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/darkie-coder/agents/` lub `.darkie-coder/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Agent domyślny musi być agentem typu `primary` (nie `subagent`). Może to być agent wbudowany, taki jak `"build"` lub `"plan"`, albo zdefiniowany przez Ciebie [agent niestandardowy](/docs/agents). Jeśli agent nie istnieje lub jest subagentem, Darkie Coder powróci do `"build"` z ostrzeżeniem.

Dotyczy to wszystkich interfejsów: TUI, CLI (`darkie-coder run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/darkie-coder/commands/` lub `.darkie-coder/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
Darkie Coder może automatycznie pobierać nowe wersje. Możesz to kontrolować za pomocą opcji `autoupdate`.
...
Działa to tylko wtedy, gdy Darkie Coder nie został zainstalowany przez menedżera pakietów, takiego jak Homebrew.
...
Domyślnie Darkie Coder **zezwala na wszystkie działania** bez konieczności zatwierdzania. Możesz to zmienić za pomocą opcji `permission`.
...
[Wtyczki](/docs/plugins) rozszerzają Darkie Coder o niestandardowe narzędzia, hooki i integracje.

Umieść pliki wtyczek w `.darkie-coder/plugins/` lub `~/.config/darkie-coder/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
...
Jest to przydatne, gdy chcesz ograniczyć użycie Darkie Coder tylko do określonych dostawców.
