---
title: Serwer
description: Interakcja z serwerem darkie-coder za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `darkie-coder serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient darkie-coder.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `darkie-coder.local` |
...
Ustaw `DARKIE_CODER_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `darkie-coder` lub ustaw `DARKIE_CODER_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `darkie-coder serve`, jak i `darkie-coder web`.
...
Kiedy uruchomisz `darkie-coder`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera darkie-coder do programowej interakcji z darkie-coder.
:::

Ta architektura umożliwia obsługę wielu klientów przez darkie-coder i programową interakcję z kodem otwartym.

Możesz uruchomić `darkie-coder serve`, aby uruchomić samodzielny serwer. Jeśli masz
darkie-coder TUI działa, `darkie-coder serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki darkie-coder [IDE](/docs/ide).
...
Serwer darkie-coder udostępnia następujące interfejsy API.
