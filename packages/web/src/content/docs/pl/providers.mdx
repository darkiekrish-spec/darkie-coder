---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w darkie-coder.
---

import config from "../../../../config.mjs"
export const console = config.console

darkie-coder używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji darkie-coder.
...
w `~/.local/share/darkie-coder/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim darkie-coder
config.
...
Darkie Coder Zen to lista modeli dostarczonych przez zespół darkie-coder, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z darkie-coder. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Darkie Coder Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Darkie Coder Zen` i przejdź do [darkie-coder.ai/auth](https://darkie-coder.ai/zen).
...
Działa jak każdy inny dostawca w darkie-coder i jest całkowicie opcjonalny w użyciu.

---

## Darkie Coder Go

Darkie Coder Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół darkie-coder, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z darkie-coder.

1. Uruchom polecenie `/connect` w TUI, wybierz `Darkie Coder Go` i przejdź do [darkie-coder.ai/auth](https://darkie-coder.ai/zen).
...
Aby używać Amazon Bedrock z darkie-coder:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania darkie-coder:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `darkie-coder.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w darkie-coder nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować darkie-coder do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod darkie-coder działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji darkie-coder.
...
darkie-coder używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować darkie-coder
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`darkie-coder.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [darkie-coder-gitlab-auth](https://www.npmjs.com/package/darkie-coder-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem darkie-coder:
...
Aby używać Google Vertex AI z darkie-coder:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania darkie-coder.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą darkie-coder, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [darkie-coder-helicone-session](https://github.com/H2Shami/darkie-coder-helicone-session), aby automatycznie rejestrować każdą konwersację darkie-coder jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja darkie-coder będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować darkie-coder tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować darkie-coder tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować darkie-coder tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem darkie-coder. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/darkie-coder).
:::
...
Aby korzystać z Ollama Cloud z darkie-coder:
...
3. Skopiuj klucz API do użycia w darkie-coder.
...
6. **Ważne**: Przed użyciem modeli chmurowych w darkie-coder musisz pobrać informacje o modelu lokalnie:
...
Darkie Coder Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół darkie-coder. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Darkie Coder Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Darkie Coder Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API darkie-coder.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji darkie-coder.
...
5. Możesz je także dostosować za pomocą konfiguracji darkie-coder. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z darkie-coder:
...
Możesz także dostosować modele za pomocą konfiguracji darkie-coder. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z darkie-coder. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `darkie-coder.json` w katalogu projektu:
...
Pola `limit` pozwalają darkie-coder zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `darkie-coder auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację darkie-coder i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji darkie-coder.
