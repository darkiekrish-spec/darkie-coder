...
Dzięki darkie-coder możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie darkie-coder używa naszego własnego motywu `darkie-coder`.
...
darkie-coder ma kilka wbudowanych motywów.
...
- Chcą, aby Darkie Coder pasował do wyglądu ich terminala
...
darkie-coder obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/darkie-coder/themes/*.json` lub `$XDG_CONFIG_HOME/darkie-coder/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.darkie-coder/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.darkie-coder/themes/*.json`
