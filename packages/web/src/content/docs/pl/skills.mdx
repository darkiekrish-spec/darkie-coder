...
Umiejętności agenta pozwalają darkie-coder odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
darkie-coder przeszukuje te lokalizacje:

- Project config: `.darkie-coder/skills/<name>/SKILL.md`
- Global config: `~/.config/darkie-coder/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu darkie-coder przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.darkie-coder/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/darkie-coder/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.darkie-coder/skills/git-release/SKILL.md` w ten sposób:
...
darkie-coder wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `darkie-coder.json`:
...
**Dla agentów wbudowanych** (w `darkie-coder.json`):
