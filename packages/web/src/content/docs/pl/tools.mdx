...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Darkie Coder posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Darkie Coder.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `DARKIE_CODER_EXPERIMENTAL_LSP_TOOL=true` (lub `DARKIE_CODER_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Darkie Coder lub gdy zmienna środowiskowa `DARKIE_CODER_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Darkie Coder:
