...
Możesz dodać dodatkowe informacje do darkie-coder za pomocą _Model Context Protocol_, w skrócie MCP. darkie-coder obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [darkie-coder Config](https://darkie-coder.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/darkie-coder`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
darkie-coder automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, darkie-coder:
...
Jeśli serwer wymaga uwierzytelnienia, darkie-coder poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `darkie-coder mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji darkie-coder bezpieczne przechowa tokeny w `~/.local/share/darkie-coder/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w darkie-coder, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie darkie-coder, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć darkie-coder z kontem Sentry.
