...
W Darkie Coder obowiązuje zasada dwóch typów agentów: Primary (Główni) i Subagents (Subagenci).
...
Darkie Coder zawiera dwa wbudowane agenty główne: **Build** i **Plan**. Przyjrzymy się im poniżej.
...
Darkie Coder ma trzech wbudowanych subagentów: **General**, **Explore** i **Scout**. Przyjrzymy się im poniżej.

---

## Wbudowane

Darkie Coder ma dwa wbudowane agenty główne i trzech wbudowanych subagentów.
...
Agent tylko do odczytu do pracy z zewnętrzną dokumentacją i badaniem zależności. Używaj go, gdy chcesz sklonować repozytorium zależności do zarządzanej pamięci podręcznej Darkie Coder, przejrzeć kod źródłowy biblioteki albo porównać lokalny kod z implementacjami upstream bez modyfikowania swojego workspace.
...
Skonfiguruj agenta w pliku `darkie-coder.json`:
...
- Globalnie: `~/.config/darkie-coder/agents/`
- W projekcie: `.darkie-coder/agents/`
...
Jeśli nie zostanie skonfigurowana, Darkie Coder używa wartości domyślnej dla danego modelu; 0 dla modeli rozumowania (reasoning models), 0.55 dla modeli Qwen.
...
Ścieżka jest względna do miejsca, w którym znajduje się plik konfiguracyjny. Działa to zarówno w przypadku globalnej konfiguracji Darkie Coder, jak i konfiguracji specyficznej dla projektu.
...
Identyfikator modelu jest taki sam, jak w konfiguracji Darkie Coder w `provider/model-id`. Na przykład, [Darkie Coder Zen](/docs/zen), użyj `darkie-coder/gpt-5.1-codex` dla GPT 5.1 Codex.
...
:::tip
Uruchom `darkie-coder models`, aby zobaczyć dostępne modele.
:::
...
:::tip
Masz agenta, którym warto się podzielić? [Prześlij PR](https://github.com/anomalyco/darkie-coder).
:::
