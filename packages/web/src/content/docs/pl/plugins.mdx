---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć darkie-coder.
---

Wtyczki umożliwiające wyłączenie darkie-coder poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania darkie-coder.
...
- `.darkie-coder/plugins/` - Wtyczki na poziomie projektu
- `~/.config/darkie-coder/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/darkie-coder/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/darkie-coder/darkie-coder.json`)
2. Konfiguracja projektu (`darkie-coder.json`)
3. Globalny katalog wtyczek (`~/.config/darkie-coder/plugins/`)
4. Katalog wtyczek projektu (`.darkie-coder/plugins/`)
...
darkie-coder uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z darkie-coder do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do darkie-coder.
...
:::note
Jeśli korzystasz z aplikacji komputerowej darkie-coder, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj darkie-coder czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do darkie-coder:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod darkie-coder. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://darkie-coder.ai/docs/sdk).
