---
title: Complementos
description: Escriba sus propios complementos para extender Darkie Coder.
---

Los complementos le permiten extender Darkie Coder al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Darkie Coder.
...
- `.darkie-coder/plugins/` - Complementos a nivel de proyecto
- `~/.config/darkie-coder/plugins/` - Complementos globales
...
```json title="darkie-coder.json"
{
  ...
  "plugin": ["darkie-coder-helicone-session", "darkie-coder-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/darkie-coder/node_modules/`.
...
1. Configuración global (`~/.config/darkie-coder/darkie-coder.json`)
2. Configuración del proyecto (`darkie-coder.json`)
3. Directorio global de complementos (`~/.config/darkie-coder/plugins/`)
4. Directorio de complementos del proyecto (`.darkie-coder/plugins/`)
...
```json title=".darkie-coder/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
Darkie Coder ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".darkie-coder/plugins/my-plugin.ts"
...
```js title=".darkie-coder/plugins/example.js"
...
- `client`: Un cliente SDK darkie-coder para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar darkie-coder.
...
```js title=".darkie-coder/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "darkie-coder"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Darkie Coder, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que darkie-coder lea archivos `.env`:

```javascript title=".darkie-coder/plugins/env-protection.js"
...
```javascript title=".darkie-coder/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a darkie-coder:

```ts title=".darkie-coder/plugins/custom-tools.ts"
...
El ayudante `tool` crea una herramienta personalizada a la que darkie-coder puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para darkie-coder junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://darkie-coder.ai/docs/sdk) para obtener más detalles.
...
```ts title=".darkie-coder/plugins/compaction.ts"
import type { Plugin } from "@opencode-ai/plugin"
...
```ts title=".darkie-coder/plugins/custom-compaction.ts"
import type { Plugin } from "@opencode-ai/plugin"
