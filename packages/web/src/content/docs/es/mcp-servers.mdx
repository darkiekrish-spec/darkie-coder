...
Puede agregar herramientas externas a Darkie Coder utilizando el _Model Context Protocol_ o MCP. Darkie Coder admite servidores locales y remotos.
...
Puede definir servidores MCP en su [Darkie Coder Config](https://darkie-coder.ai/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/darkie-coder`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
Darkie Coder maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, Darkie Coder:
...
Si el servidor requiere autenticación, Darkie Coder le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `darkie-coder mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, Darkie Coder almacenará los tokens de forma segura en `~/.local/share/darkie-coder/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en Darkie Coder, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración Darkie Coder como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar Darkie Coder a su cuenta Sentry.
