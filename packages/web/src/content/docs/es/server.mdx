---
title: Servidor
description: Interactuar con el servidor darkie-coder a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `darkie-coder serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente darkie-coder puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `darkie-coder.local` |
...
Configure `DARKIE_CODER_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `darkie-coder`, o configure `DARKIE_CODER_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `darkie-coder serve` como a `darkie-coder web`.
...
Cuando ejecuta `darkie-coder`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor darkie-coder para interactuar con darkie-coder mediante programación.
:::

Esta arquitectura permite que darkie-coder admita múltiples clientes y le permite interactuar con darkie-coder mediante programación.

Puede ejecutar `darkie-coder serve` para iniciar un servidor independiente. Si tienes el
darkie-coder TUI ejecutándose, `darkie-coder serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Darkie Coder [IDE](/docs/ide).
...
El servidor darkie-coder expone las siguientes API.
