...
description: Cree herramientas que LLM pueda llamar en darkie-coder.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de darkie-coder como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.darkie-coder/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/darkie-coder/tools/`.
...
```ts title=".darkie-coder/tools/database.ts" {1}
...
```ts title=".darkie-coder/tools/math.ts"
import { tool } from "@opencode-ai/plugin"
...
```ts title=".darkie-coder/tools/bash.ts"
import { tool } from "@opencode-ai/plugin"
...
```ts title=".darkie-coder/tools/project.ts" {8}
import { tool } from "@opencode-ai/plugin"
...
```python title=".darkie-coder/tools/add.py"
import sys
...
```ts title=".darkie-coder/tools/python-add.ts" {10}
import { tool } from "@opencode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".darkie-coder/tools/add.py")
