...
Las herramientas permiten que LLM realice acciones en su código base. Darkie Coder viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Darkie Coder.
...
```json title="darkie-coder.json" {4}
{
  "$schema": "https://darkie-coder.ai/config.json",
  "permission": {
    "bash": "allow"
...
:::note
Esta herramienta solo está disponible cuando `DARKIE_CODER_EXPERIMENTAL_LSP_TOOL=true` (o `DARKIE_CODER_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Darkie Coder o cuando la variable de entorno `DARKIE_CODER_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Darkie Coder:

```bash
DARKIE_CODER_ENABLE_EXA=1 darkie-coder
```
