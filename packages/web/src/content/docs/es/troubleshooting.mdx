...
Para depurar problemas con Darkie Coder, comience verificando los registros y los datos locales que almacena en el disco.
...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\darkie-coder\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `darkie-coder --log-level DEBUG`.

---

## Almacenamiento

darkie-coder almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/darkie-coder/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\darkie-coder`
...
Darkie Coder Desktop ejecuta un servidor Darkie Coder local (el sidecar `darkie-coder-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `Darkie Coder` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/darkie-coder/darkie-coder.jsonc` (o `~/.config/darkie-coder/darkie-coder.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/darkie-coder/darkie-coder.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\darkie-coder\darkie-coder.jsonc`
...
Darkie Coder también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/darkie-coder/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\darkie-coder\plugins`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que Darkie Coder pueda reconstruirlo.

1. Salga de Darkie Coder Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/darkie-coder`
- **Linux**: eliminar `~/.cache/darkie-coder` (o ejecutar `rm -rf ~/.cache/darkie-coder`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\darkie-coder`

3. Reinicie el escritorio Darkie Coder.

---

### Solucionar problemas de conexión del servidor

Darkie Coder Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `darkie-coder.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `DARKIE_CODER_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `DARKIE_CODER_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio Darkie Coder requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de Darkie Coder.

---

### Notificaciones que no se muestran

Darkie Coder Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para Darkie Coder en la configuración de su sistema operativo, y
...
1. Salga del escritorio Darkie Coder.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio Darkie Coder):

- `darkie-coder.settings.dat` (URL del servidor predeterminado de escritorio)
- `darkie-coder.global.dat` y `darkie-coder.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con Darkie Coder:
...
3. Asegúrese de tener la última versión con `darkie-coder upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `darkie-coder models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\darkie-coder`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. darkie-coder instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\darkie-coder`

2. Reinicie darkie-coder para reinstalar los paquetes del proveedor más recientes.

Esto obligará a darkie-coder a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
darkie-coder detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
