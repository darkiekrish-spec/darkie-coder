...
Las habilidades del agente permiten a Darkie Coder descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Darkie Coder busca estas ubicaciones:

- Configuración del proyecto: `.darkie-coder/skills/<name>/SKILL.md`
- Configuración global: `~/.config/darkie-coder/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Darkie Coder sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.darkie-coder/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/darkie-coder/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.darkie-coder/skills/git-release/SKILL.md` así:
...
Darkie Coder enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `darkie-coder.json`:
...
**Para agentes integrados** (en `darkie-coder.json`):
