...
describe("inlineCodeKind", () => {
  ...
  test("detects file and directory paths", () => {
    ...
    expect(inlineCodeKind(`~/.config/darkie-coder`)).toBe("path")
    expect(inlineCodeKind(`@opencode-ai/app`)).toBe("path")
    expect(inlineCodeKind(`session/status`)).toBe("path")
  })

  test("detects urls", () => {
    expect(inlineCodeKind(`https://darkie-coder.ai/docs`)).toBe("url")
    expect(inlineCodeKind(`http://localhost:4444`)).toBe("url")
    expect(inlineCodeKind(`file:///tmp/darkie-coder`)).toBeUndefined()
    expect(inlineCodeKind(`ftp://darkie-coder.ai/docs`)).toBeUndefined()
  })
})
