...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("darkie-coder-find")
  api.delete("darkie-coder-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("darkie-coder-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("darkie-coder-find", new Highlight(...rest))
    return true
  }
