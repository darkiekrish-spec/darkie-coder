...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve darkie-coder route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "darkie-coder")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "darkie-coder")).toBe("openai")
    expect(statProvider("big-pickle", "", "darkie-coder")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "darkie-coder"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep darkie-coder as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "darkie-coder"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "darkie-coder"))).toMatchObject([{ provider: "unknown" }])
    expect(toProviderAggregate(aggregate("muse-spark-1.2-contributor", "unknown"))).toMatchObject([
      { provider: "meta" },
    ])
  })

  test("geo aggregates never keep darkie-coder or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "darkie-coder"), country: "US" })).toMatchObject([
