...
import { getDarkie CoderModeStack, DARKIE_CODER_BASE_MODE, Darkie CoderKeymapProvider, registerDarkie CoderKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerDarkie CoderKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when darkie-coder mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getDarkie CoderModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getDarkie CoderModeStack(keymap).push("autocomplete")
