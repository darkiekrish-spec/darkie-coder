...
import type { useDarkie CoderKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useDarkie CoderKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getDarkie CoderModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getDarkie CoderModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
