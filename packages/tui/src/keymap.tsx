...
export const DARKIE_CODER_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const DARKIE_CODER_MODE_KEY = "darkie-coder.mode"

export const Darkie CoderKeymapProvider = KeymapProvider
export const useDarkie CoderKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type Darkie CoderModeStack = ReturnType<typeof createDarkie CoderModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, Darkie CoderModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createDarkie CoderModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(DARKIE_CODER_MODE_KEY, DARKIE_CODER_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(DARKIE_CODER_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(DARKIE_CODER_MODE_KEY, stack.at(-1)?.mode ?? DARKIE_CODER_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? DARKIE_CODER_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(DARKIE_CODER_MODE_KEY, undefined)
...
export function useDarkie CoderModeStack() {
  return getDarkie CoderModeStack(useDarkie CoderKeymap())
}

export function getDarkie CoderModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Darkie Coder mode stack is not registered for this keymap")
  return value
}
...
export function registerDarkie CoderKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createDarkie CoderModeStack(keymap)
...
export function useCommandSlashes(): Accessor<readonly CommandSlashEntry[]> {
  const keymap = useDarkie CoderKeymap()
