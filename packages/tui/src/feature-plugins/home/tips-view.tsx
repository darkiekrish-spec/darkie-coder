...
export function Tips(props: { api: TuiPluginApi; connected?: boolean }) {
  ...
  const shortcuts: Shortcuts = {
    ...
    statusView: useCommandShortcut("darkie-coder.status"),
...
const TIPS: Tip[] = [
  ...
  "Run {highlight}/share{/highlight} to create a public darkie-coder.ai link",
  ...
  "Create {highlight}darkie-coder.json{/highlight} for server settings, and {highlight}tui.json{/highlight} for TUI",
  "Place TUI settings in {highlight}~/.config/darkie-coder/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.darkie-coder/commands/{/highlight} for reusable prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.darkie-coder/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.darkie-coder/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.darkie-coder/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent Darkie Coder from reading sensitive files",
  "Use {highlight}darkie-coder run{/highlight} for non-interactive scripting",
  "Use {highlight}darkie-coder --continue{/highlight} to resume the last session",
  "Use {highlight}darkie-coder run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}darkie-coder serve{/highlight} for headless API access to Darkie Coder",
  "Use {highlight}darkie-coder run --attach{/highlight} to connect to a running server",
  "Run {highlight}darkie-coder upgrade{/highlight} to update to the latest version",
  "Run {highlight}darkie-coder auth list{/highlight} to see all configured providers",
  "Run {highlight}darkie-coder agent create{/highlight} for guided agent creation",
  "Use {highlight}/darkie-coder{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}darkie-coder github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/darkie-coder fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.darkie-coder/themes/{/highlight} directory",
  ...
  "Run {highlight}darkie-coder debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/anomalyco/darkie-coder{/highlight} in a container",
  "Use {highlight}/connect{/highlight} with Darkie Coder Zen for curated, tested models",
