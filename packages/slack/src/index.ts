...
import { createDarkie Coder, type ToolPart } from "@opencode-ai/sdk"
...
console.log("🚀 Starting darkie-coder server...")
const darkie-coder = await createDarkie Coder({
  port: 0,
})
console.log("✅ Darkie Coder server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await darkie-coder.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new darkie-coder session...")
    const { client, server } = darkie-coder
    ...
    console.log("✅ Created darkie-coder session:", createResult.data.id)
  ...
  console.log("📝 Sending to darkie-coder:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Darkie Coder response:", JSON.stringify(result, null, 2))
