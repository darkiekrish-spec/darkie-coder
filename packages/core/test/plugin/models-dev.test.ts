...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.DARKIE_CODER_MODELS_PATH,
          disabled: Flag.DARKIE_CODER_DISABLE_MODELS_FETCH,
        }
        Flag.DARKIE_CODER_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.DARKIE_CODER_MODELS_PATH = previous.path
          Flag.DARKIE_CODER_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
