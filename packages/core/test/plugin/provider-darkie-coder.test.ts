...
import { Darkie CoderPlugin } from "@opencode-ai/core/plugin/provider/darkie-coder"
...
const addPlugin = Effect.fn(function* (http?: HttpClient.HttpClient) {
  ...
  yield* Darkie CoderPlugin.effect(host).pipe(
...
describe("Darkie CoderPlugin", () => {
  it.effect("registers account and service account methods", () =>
    Effect.gen(function* () {
      yield* addPlugin()
      expect((yield* (yield* Integration.Service).get(Integration.ID.make("darkie-coder")))?.methods).toEqual([
        {
          id: Integration.MethodID.make("device"),
          type: "oauth",
          label: "Darkie Coder Console account",
        },
  ...
  it.effect("resolves origin-rooted device verification URLs", () =>
    Effect.gen(function* () {
        ...
        Effect.succeed(
          HttpClientResponse.fromWeb(
            ...
            Response.json({
              device_code: "device",
              user_code: "user",
              verification_uri_complete: "/console/device?user_code=user&client_id=darkie-coder-cli",
              expires_in: 60,
              interval: 60,
            }),
          ),
      ...
      const attempt = yield* integration.connection.oauth({
        integrationID: Integration.ID.make("darkie-coder"),
        methodID: Integration.MethodID.make("device"),
        inputs: {},
      })
      expect(attempt.url).toBe("https://darkie-coder.ai/console/device?user_code=user&client_id=darkie-coder-cli")
    }),
  )
  ...
  it.live("loads providers and models from the connected Darkie Coder server", () =>
        ...
        Effect.gen(function* () {
          ...
          const provider = required(
            yield* eventually(
              ...
              (item) => item?.integrationID === Integration.ID.make("darkie-coder"),
            ),
          )
          expect(provider).toMatchObject({
            name: "Remote",
            integrationID: "darkie-coder",
            api: {
              type: "aisdk",
  ...
  it.effect("uses a public key and disables paid models without credentials", () =>
    withEnv({ DARKIE_CODER_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
            ...ProviderV2.Info.empty(ProviderV2.ID.darkie-coder),
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(ProviderV2.ID.darkie-coder)).request.body.apiKey).toBe("public")
        expect(required(yield* catalog.model.get(ProviderV2.ID.darkie-coder, ModelV2.ID.make("paid"))).enabled).toBe(false)
      }),
    ),
  )
  ...
  it.effect("keeps free models without credentials", () =>
    withEnv({ DARKIE_CODER_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(ProviderV2.ID.darkie-coder)).request.body.apiKey).toBe("public")
        expect(required(yield* catalog.model.get(ProviderV2.ID.darkie-coder, ModelV2.ID.make("free"))).enabled).toBe(true)
      }),
    ),
  )
  ...
  it.effect("treats output-only cost as free without credentials", () =>
    withEnv({ DARKIE_CODER_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(ProviderV2.ID.darkie-coder)).request.body.apiKey).toBe("public")
        expect(required(yield* catalog.model.get(ProviderV2.ID.darkie-coder, ModelV2.ID.make("output-only"))).enabled).toBe(
          true,
        )
      }),
    ),
  ...
  it.effect("uses DARKIE_CODER_API_KEY as credentials", () =>
    withEnv({ DARKIE_CODER_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(ProviderV2.ID.darkie-coder)).request.body.apiKey).toBeUndefined()
        expect(required(yield* catalog.model.get(ProviderV2.ID.darkie-coder, ModelV2.ID.make("paid"))).enabled).toBe(true)
      }),
    ),
  )

  it.effect("uses configured provider env vars as credentials", () =>
    withEnv({ DARKIE_CODER_API_KEY: undefined, CUSTOM_DARKIE_CODER_API_KEY: "secret" }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        ...
        yield* integrations.transform((editor) => {
          editor.method.update({
            integrationID: Integration.ID.make("darkie-coder"),
            method: { type: "env", names: ["CUSTOM_DARKIE_CODER_API_KEY"] },
          })
        })
        yield* catalog.transform((catalog) => {
          const provider = ProviderV2.Info.make({
  ...
  it.effect("uses configured apiKey as credentials", () =>
    withEnv({ DARKIE_CODER_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        yield* addPlugin()
        expect(required(yield* catalog.provider.get(ProviderV2.ID.darkie-coder)).request.body.apiKey).toBe("configured")
        expect(required(yield* catalog.model.get(ProviderV2.ID.darkie-coder, ModelV2.ID.make("paid"))).enabled).toBe(true)
      }),
    ),
  )

  it.effect("ignores non-darkie-coder providers and models", () =>
    withEnv({ DARKIE_CODER_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        const catalog = yield* Catalog.Service
        yield* catalog.transform((catalog) => {
  ...
  it.effect("prefers gpt-5-nano as the darkie-coder small model", () =>
    Effect.gen(function* () {
      const catalog = yield* Catalog.Service
      const providerID = ProviderV2.ID.darkie-coder
