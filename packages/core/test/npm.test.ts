...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@darkie-coder/acme")).toBe("@darkie-coder/acme")
    expect(Npm.sanitize("@darkie-coder/acme@1.0.0")).toBe("@darkie-coder/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/darkie-coder/acme.git"
    const expected = win ? "acme@git+https_//github.com/darkie-coder/acme.git" : spec
