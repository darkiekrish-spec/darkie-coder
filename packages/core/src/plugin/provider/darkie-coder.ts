...
const defaultServer = "https://darkie-coder.ai/console"
const clientID = "darkie-coder-cli"
...
export const Darkie CoderPlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "darkie-coder",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("Darkie CoderPlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("darkie-coder")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load Darkie Coder provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("darkie-coder", (integration) => {
        integration.name = "Darkie Coder"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "darkie-coder", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("darkie-coder")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("darkie-coder")
      ...
      const item = catalog.provider.get(ProviderV2.ID.darkie-coder)
      if (!item) return
      const hasKey = Boolean(process.env.DARKIE_CODER_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("darkie-coder")),
