...
export function fileLogger(file = path.join(Global.Path.log, "darkie-coder.log"), id: string = runID) {
...
export function minimumLogLevel() {
  const value = process.env.DARKIE_CODER_LOG_LEVEL?.toUpperCase()
  const levels = {
    DEBUG: "Debug",
    INFO: "Info",
    WARN: "Warn",
...
export function loggers() {
  return process.env.DARKIE_CODER_PRINT_LOGS === "1" ? [fileLogger(), stderrLogger] : [fileLogger()]
}
