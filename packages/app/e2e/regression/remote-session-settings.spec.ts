...
async function configureServers(page: Page, tabs: { type: "session"; server: string; sessionId: string }[] = []) {
  await page.addInitScript(
    ({ serverB, tabs }) => {
      ...
      localStorage.setItem("darkie-coder.global.dat:server", JSON.stringify({ list: [serverB] }))
      localStorage.setItem("darkie-coder.window.browser.dat:tabs", JSON.stringify(tabs))
    },
    { serverB, tabs },
  )
}
