...
import { currentSession, mockDarkie CoderServer } from "../utils/mock-server"
import { expectSessionTitle } from "../utils/waits"

const directory = "C:/Darkie Coder/SubagentNavigation"
...
test("navigates to a subagent child session missing from the session list", async ({ page }) => {
  ...
  const titlebarRight = page.locator("#darkie-coder-titlebar-right")
...
async function setup(page: Page, events?: () => EventPayload[]) {
  await mockDarkie CoderServer(page, {
    ...
    provider: {
      ...
      default: { providerID: "darkie-coder", modelID: "claude-opus-4-6" },
    },
  ...
  return [
    {
      info: {
        ...
        agent: "build",
        model: { providerID: "darkie-coder", modelID: "claude-opus-4-6" },
      },
      parts: [
        {
...
async function configurePage(page: Page) {
  ...
  await page.addInitScript(
    ({ directory, server, sessionId }) => {
      ...
      localStorage.setItem("darkie-coder.window.browser.dat:tabs", JSON.stringify([{ type: "session", server, sessionId }]))
    },
