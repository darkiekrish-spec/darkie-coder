...
const directory = "C:/Darkie Coder/NewSessionPanelCorner"
...
test("matches the rounded panel corners to the dark new-session background", async ({ page }, testInfo) => {
  ...
  await page.addInitScript(
    ({ directory, draftID, server }) => {
      ...
      localStorage.setItem("darkie-coder-theme-id", "oc-2")
      localStorage.setItem("darkie-coder-color-scheme", "dark")
