...
export const directory = "C:/Darkie Coder/TimelineStability"
...
export const model = { providerID: "darkie-coder", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("darkie-coder.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
