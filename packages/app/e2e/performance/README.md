...
Set `DARKIE_CODER_PERFORMANCE_TRACE_DIR` to emit a standard Chrome DevTools trace for every benchmark page automatically:

```sh
DARKIE_CODER_PERFORMANCE_TRACE_DIR=/tmp/darkie-coder-performance-traces \
...
INP analysis requires a trace with a supported navigation/interaction insight. Selector statistics require a trace captured with `DARKIE_CODER_PERFORMANCE_SELECTOR_TRACE=1`.
