...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when Darkie Coder needs attention", () => {
    expect(wslDarkie CoderAction(undefined)).toBeUndefined()
    expect(
      wslDarkie CoderAction({
        distro: "Debian",
        resolvedPath: null,
    ...
    ).toBe("wsl.onboarding.installDarkie Coder")
    expect(
      wslDarkie CoderAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/darkie-coder",
        version: "1.2.2",
        expectedVersion: "1.2.3",
        matchesDesktop: false,
        error: null,
      }),
    ).toBe("wsl.onboarding.updateDarkie Coder")
    expect(
      wslDarkie CoderAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/darkie-coder",
  ...
  test("does not report ready when Darkie Coder is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        darkie-coderChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.darkie-coder/bin/darkie-coder",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
            error: "darkie-coder is installed but could not run",
          },
        },
      },
      view: "main",
    ...
    expect(model.distroStatuses.Debian).toEqual({
      label: { key: "wsl.onboarding.installDarkie Coder" },
      tone: "warning",
    })
    expect(model.primaryButton.action).toBe("install-darkie-coder")
  })
