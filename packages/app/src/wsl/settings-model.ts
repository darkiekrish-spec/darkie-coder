...
export type AddServerPrimaryButton = {
  ...
  label: AddServerText
  disabled: boolean
  action: "install-darkie-coder" | "add" | null
  loading: boolean
  width: string | null
}
...
export function wslDarkie CoderAction(check?: WslDarkie CoderCheck) {
  if (!check) return
  if (!check.resolvedPath) return "wsl.onboarding.installDarkie Coder"
  if (check.matchesDesktop === false) return "wsl.onboarding.updateDarkie Coder"
}
...
export function addServerViewModel(input: {
  ...
  const darkie-coderCheck = selectedDistro ? (state?.darkie-coderChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.darkie-coderChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (check.matchesDesktop === false) return { label: { key: "wsl.onboarding.updateDarkie Coder" }, tone: "warning" }
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.darkie-coderMissing" }, tone: "warning" }
  if (check.error) return { label: { key: "wsl.onboarding.installDarkie Coder" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  darkie-coderCheck: WslDarkie CoderCheck | null
  adding: boolean
  probingAddable: boolean
  ...
  const probingDarkie Coder =
    probingSelected ||
    (ready &&
      (!input.darkie-coderCheck ||
        (!!input.selectedDistro &&
          input.state?.job?.kind === "probe-addable" &&
          input.state.job.distros.includes(input.selectedDistro))))
  const installingDarkie Coder =
    input.state?.job?.kind === "install-darkie-coder" && input.state.job.distro === input.selectedDistro
  if (!ready || probingDarkie Coder) {
    return {
      variant: "contrast",
  ...
  if (!addServerDarkie CoderReady(input.darkie-coderCheck)) {
    const update = !!input.darkie-coderCheck?.resolvedPath && input.darkie-coderCheck.matchesDesktop === false
    return {
      variant: "neutral",
      label: installingDarkie Coder
        ? { key: "wsl.onboarding.updatingDarkie Coder" }
        : update
          ? { key: "wsl.onboarding.updateDarkie Coder" }
          : { key: "wsl.onboarding.installDarkie Coder" },
      disabled: !!input.state?.job || input.adding,
      action: "install-darkie-coder",
      loading: installingDarkie Coder,
...
function addServerDarkie CoderReady(check: WslDarkie CoderCheck | null) {
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.darkie-coderChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.darkie-coderChecks[item.name]) return [`darkie-coder:${item.name}`]
    return []
  })
