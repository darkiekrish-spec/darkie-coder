...
import { wslDarkie CoderAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.darkie-coderChecks[item.config.distro]
          const darkie-coderAction = () => wslDarkie CoderAction(check())
          const busy = () => wsl.data?.job?.kind === "install-darkie-coder" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={darkie-coderAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installDarkie Coder(item.config.distro))}
                    >
