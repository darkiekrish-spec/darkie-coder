...
function ProviderPicker(props: {
  ...
  const note = (id: string) => {
    ...
    if (id === "darkie-coder-go") return language.t("dialog.provider.darkie-coderGo.tagline")
    return undefined
  }
    ...
    >
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <span>{i.name}</span>
          <Show when={i.id === "darkie-coder"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.darkie-coder.tagline")}</div>
          </Show>
          ...
          <Show when={i.id === "darkie-coder-go"}>
            <Tag>{language.t("dialog.provider.tag.recommended")}</Tag>
          </Show>
        </div>
      )}
...
function ProviderPickerV2(props: {
  ...
  const featured = ["darkie-coder", "darkie-coder-go", "anthropic", "openai", "google", "openrouter", "vercel"]
                  ...
                  <For each={group.items()}>
                    {(provider) => (
                      ...
                      >
                        ...
                        <Show when={provider.id === "darkie-coder" || provider.id === "darkie-coder-go"}>
                          <span class="min-w-0 truncate font-[440] text-v2-text-text-muted">
                            {language.t(
                              provider.id === "darkie-coder"
                                ? "dialog.provider.darkie-coder.tagline"
                                : "dialog.provider.darkie-coderGo.tagline",
                            )}
                          </span>
    ...
    if (method.type === "oauth") {
      ...
      await serverSDK()
        ...
        .then((x) => {
          ...
          if (props.provider === "darkie-coder" && platform.platform === "desktop") {
            const url = new URL(x.data.url)
            url.searchParams.set("client_id", "darkie-coder-desktop")
            x.data.url = url.href
          }
  ...
  function ApiAuthView() {
    ...
    if (newLayout())
      return (
        <div class="flex flex-col gap-5 px-3 text-[13px] font-[440] leading-5 tracking-[-0.04px] text-v2-text-text-muted">
          <Show
            when={provider().id === "darkie-coder"}
            fallback={language.t("provider.connect.apiKey.description", { provider: provider().name })}
          >
            <div class="flex flex-col gap-5">
              <div>{language.t("provider.connect.darkie-coderZen.line1")}</div>
              <div>{language.t("provider.connect.darkie-coderZen.line2")}</div>
              <div>
                {language.t("provider.connect.darkie-coderZen.visit.prefix")}
                <ExternalLink
                  href="https://darkie-coder.ai/zen"
                  class="text-v2-text-text-base focus-visible:rounded-xs focus-visible:outline-2 focus-visible:outline-v2-border-border-focus"
                >
                  {language.t("provider.connect.darkie-coderZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.darkie-coderZen.visit.suffix")}
              </div>
            </div>
          </Show>
    ...
    return (
      <div class="flex flex-col gap-6">
        <Switch>
          <Match when={provider().id === "darkie-coder"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.darkie-coderZen.line1")}</div>
              <div class="text-14-regular text-text-base">{language.t("provider.connect.darkie-coderZen.line2")}</div>
              <div class="text-14-regular text-text-base">
                {language.t("provider.connect.darkie-coderZen.visit.prefix")}
                <ExternalLink href="https://darkie-coder.ai/zen" tabIndex={-1}>
                  {language.t("provider.connect.darkie-coderZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.darkie-coderZen.visit.suffix")}
              </div>
