...
import type { Darkie CoderEvent, SessionApi } from "@opencode-ai/client/promise"
import type { Message, Darkie CoderClient, Part, Session } from "@opencode-ai/sdk/v2/client"
...
function messageClient(...responses: Array<MessageResponse | Promise<MessageResponse>>) {
  ...
  } as unknown as Darkie CoderClient
...
describe("server session", () => {
  test("projects V2 session events into current and legacy message state", () => {
    ...
    const apply = (input: object) => ctx.store.applyV2(input as Darkie CoderEvent)
  ...
  test("extends a current page to include the user for split assistant turns", async () => {
    ...
    const store = createServerSession({} as Darkie CoderClient, {} as SessionApi, messageApi)
    store.remember(session("root"))
