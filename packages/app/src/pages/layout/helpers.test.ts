...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("darkie-coder://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("darkie-coder://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("darkie-coder://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("darkie-coder://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("darkie-coder://open-project")).toBeUndefined()
    expect(parseDeepLink("darkie-coder://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "darkie-coder://open-project?directory=/a",
      "darkie-coder://other?directory=/b",
      "darkie-coder://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("darkie-coder://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("darkie-coder://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("darkie-coder://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("darkie-coder://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "darkie-coder://new-session?directory=/a",
      "darkie-coder://open-project?directory=/b",
      "darkie-coder://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __DARKIE_CODER__: {
        deepLinks: ["darkie-coder://open-project?directory=/a"],
      },
    } as unknown as Window & { __DARKIE_CODER__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["darkie-coder://open-project?directory=/a"])
