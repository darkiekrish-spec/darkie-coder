# Darkie Coder Session Runtime

Darkie Coder sessions preserve durable conversational history while assembling the runtime context an agent needs to act correctly in its current environment.
...
A temporary file created under Darkie Coder's shared tool-output directory to retain complete output that was too large for Session history.
...
**Darkie Coder Client**:
The generated Promise and Effect APIs derived from the public `HttpApi`; **Embedded Darkie Coder** shares the Effect API through an in-memory `HttpClient` against the same router and handlers.
...
**Embedded Darkie Coder**:
A scoped in-process host that structurally extends the **Darkie Coder Client**, supplies an in-memory HTTP transport, and exposes additional same-process capabilities directly.
...
- The **PTY Environment** is a server concern rather than a Core PTY concern. PTY creation merges caller values, then the host overlay, then Core-forced terminal invariants such as `TERM` and `DARKIE_CODER_TERMINAL`.
- Networked and **Embedded Darkie Coder** use the same **Darkie Coder Client** and preserve the full HTTP encoding, routing, middleware, and decoding boundary; only the `HttpClient` transport differs.
...
- Creating **Embedded Darkie Coder** is scoped. Closing its owning Scope releases the in-process server resources, database resources, registrations, and fibers.
- **Embedded Darkie Coder** exposes shared client capabilities and embedded-only capabilities on one object; consumers do not navigate through a nested `.client` property.
- The beta **Darkie Coder Client** currently uses plural consumer-facing capability groups such as `sessions`; whether the stable Session namespace should instead be singular `session` must be settled before stabilization. Internal server identifiers do not implicitly define public client names.
- Server's concrete `HttpApi` is authoritative for shared **Darkie Coder Client** capabilities. Codegen compiles its Session group directly; the Effect runtime uses an equivalent Protocol-only projection so generated artifacts remain independent of Core and Server.
...
- A capability intended for both networked and **Embedded Darkie Coder** belongs in the authoritative public `HttpApi`; embedded-only same-process capabilities extend **Embedded Darkie Coder** separately.
...
- The initial common Darkie Coder Client does not expose server-global event aggregation. `events.subscribe()` is bounded to the connected Darkie Coder instance or workspace; any future cross-instance administrative stream requires a separately designed API.
...
- The stable `sessions.list(...)` design returns a **Page** in both networked and **Embedded Darkie Coder**; embedded execution does not define a separate unbounded array-returning list operation. The beta client currently preserves the existing HTTP `{ data, cursor }` envelope until emitter-level Page projection is implemented.
...
- `sessions.create(...)` accepts an optional `location`. Omission resolves through the connected Darkie Coder instance's default or current location; an explicit value selects a known location. Networked and embedded transports use the same handler semantics.
...
- The **Embedded Darkie Coder** Layer delegates to the same scoped creation path; it does not define a second implementation.
...
- Ambient project instruction discovery honors `DARKIE_CODER_DISABLE_PROJECT_CONFIG`; global instructions remain eligible.
