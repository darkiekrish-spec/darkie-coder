...
import { createDarkie Coder } from "@opencode-ai/sdk"
...
`)
  ...
  const darkie-coder = await createDarkie Coder({ port: 0 })
  ...
  try {
    ...
    const session = await darkie-coder.client.session.create()
    const result = await darkie-coder.client.session
      .prompt({
