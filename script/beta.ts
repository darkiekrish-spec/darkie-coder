...
const model = "darkie-coder/deepseek-v4-flash"
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with darkie-coder...`)
  ...
  try {
    await $`darkie-coder run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  darkie-coder failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with darkie-coder")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with darkie-coder...")
