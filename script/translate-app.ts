...
export function parseTranslationArgs(args: string[]) {
  const parsed = parseArgs({
    ...
    options: {
      ...
      model: { type: "string", default: "darkie-coder/gpt-5.5" },
...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".darkie-coder/glossary/zh-cn.md"
  if (locale === "zht") return ".darkie-coder/glossary/zh-tw.md"
  return `.darkie-coder/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("Darkie Coder did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages))
    throw new Error("Darkie Coder returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("Darkie Coder session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel Darkie Coder runs for 'all' (default: 4)
      --model <provider/id>  Darkie Coder model (default: darkie-coder/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running Darkie Coder
...
`)
  ...
  if (failed.length) console.error(`\nDarkie Coder failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  env.DARKIE_CODER_DISABLE_PROJECT_CONFIG = "1"
  env.DARKIE_CODER_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
  ...
  const exported = Bun.spawn(["darkie-coder", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.DARKIE_CODER_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["darkie-coder", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
...
function isolatedEnvironment() {
  ...
  delete env.DARKIE_CODER_CONFIG
  delete env.DARKIE_CODER_CONFIG_DIR
  delete env.DARKIE_CODER_CONFIG_CONTENT
  delete env.DARKIE_CODER_PERMISSION
  delete env.DARKIE_CODER_AUTO_SHARE
  return env
}
