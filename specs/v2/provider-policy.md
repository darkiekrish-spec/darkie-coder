...
Both `action` and `resource` use darkie-coder's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.darkie-coder` files is intentionally deferred until `.darkie-coder` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether darkie-coder permits an operation through its managed execution paths.
