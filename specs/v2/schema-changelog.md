...
This document covers meaningful contract changes introduced on the `feat/darkie-coder-embedded-api` branch since its divergence from `origin/dev`. Mechanical file moves and internal refactors are omitted unless they changed stored data, replay behavior, public HTTP or SDK shapes, or model-facing tool contracts.
...
- Spill oversized model-facing tool text into globally unique files under Darkie Coder's shared tool-output directory.
...
- Canonicalize ambient instruction traversal boundaries, honor `DARKIE_CODER_DISABLE_PROJECT_CONFIG`, and make non-empty aggregate updates explicitly supersede previously loaded instructions.
