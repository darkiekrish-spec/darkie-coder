# Remove `packages/darkie-coder/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/darkie-coder/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the darkie-coder legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/darkie-coder/src/storage/db.ts` are concentrated in 22 source files:

- `packages/darkie-coder/src/account/repo.ts`
- `packages/darkie-coder/src/cli/cmd/db.ts`
- `packages/darkie-coder/src/cli/cmd/import.ts`
- `packages/darkie-coder/src/cli/cmd/stats.ts`
- `packages/darkie-coder/src/control-plane/workspace.ts`
- `packages/darkie-coder/src/index.ts`
- `packages/darkie-coder/src/node.ts`
- `packages/darkie-coder/src/permission/index.ts`
- `packages/darkie-coder/src/project/project.ts`
- `packages/darkie-coder/src/server/projectors.ts`
- `packages/darkie-coder/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/darkie-coder/src/server/shared/fence.ts`
- `packages/darkie-coder/src/session/message-v2.ts`
- `packages/darkie-coder/src/session/projectors.ts`
- `packages/darkie-coder/src/session/prompt.ts`
- `packages/darkie-coder/src/session/session.ts`
- `packages/darkie-coder/src/session/todo.ts`
- `packages/darkie-coder/src/share/share-next.ts`
- `packages/darkie-coder/src/storage/db.ts`
- `packages/darkie-coder/src/sync/index.ts`
- `packages/darkie-coder/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy darkie-coder database wrapper; `packages/darkie-coder/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the darkie-coder wrapper.
...
Status: Completed. `SyncEvent` and the darkie-coder projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy darkie-coder database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy darkie-coder database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy darkie-coder database wrapper.
...
All migration groups are complete or superseded. `packages/darkie-coder/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No darkie-coder data-migration group remains.

The previous darkie-coder `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate darkie-coder data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/darkie-coder`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/darkie-coder/src`
- `bun typecheck` from `packages/darkie-coder`
- Relevant package tests from `packages/darkie-coder`, not the repo root
