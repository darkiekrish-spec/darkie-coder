...
Create a small workspace package that vendors the Drizzle `effect-sqlite` adapter shape for our repo. This is not an darkie-coder storage abstraction. It is a local package that ports the Drizzle Effect SQLite implementation so we can use it before/independently of upstream release timing.

`packages/darkie-coder` will use it internally, but the package itself should be generic: Drizzle + Effect + SQLite. No darkie-coder paths, migrations, tables, transaction hooks, post-commit behavior, or domain language should live in this package.
...
- Darkie Coder-specific path/channel selection stays in `packages/darkie-coder`.

## Darkie Coder Adoption Notes

These are not package requirements, but they matter for the later darkie-coder adoption PR.

The current `packages/darkie-coder/src/storage/db.ts` has two non-obvious semantics that the darkie-coder wrapper must preserve when it consumes this adapter:
...
The darkie-coder wrapper can implement that using Effect context instead of `LocalContext`:
...
Do not remove this behavior while moving darkie-coder to Effect SQLite. `SyncEvent.run` depends on transaction composability and `behavior: "immediate"` for sequencing correctness.
...
4. Add `@opencode-ai/effect-drizzle-sqlite` as a dependency of `packages/darkie-coder`.
5. Port `packages/darkie-coder/src/storage/db.ts` to be a thin compatibility wrapper over the adapter plus darkie-coder-specific transaction/post-commit context.
...
8. Only then build domain stores like session/message/project stores on top of darkie-coder's storage wrapper.
...
An Effect Drizzle SQLite package lets us vendor the adapter once. Then darkie-coder can build its own storage wrapper on top, and `SessionStorage`, `MessageStorage`, event store, and projector writes can all share the same transaction and migration model.
...
- Should `afterCommit` stay darkie-coder-specific until event publishing moves? Default answer: yes.
...
- Do CLI/admin raw SQL and sqlite shell stay in `packages/darkie-coder`, or does the storage package expose backend capabilities for them?
...
- Use a tiny test schema, not darkie-coder domain tables.
- Prove Effect Drizzle SQLite queries, transactions, and migrations.
- Do not migrate `packages/darkie-coder` yet except possibly adding the dependency if needed for typechecking.

That gives us a focused place to validate the Effect SQLite approach before disturbing darkie-coder's current database runtime.
